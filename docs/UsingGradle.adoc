...
* *`shadowJar`* +
Creates the `expensetracker.jar` file in the `build/jar` folder, _if the current file is outdated_. +
...
Runs all tests.
* **`guiTests`** +
Runs all tests in the `seedu.expensetracker.ui` and `systemtests` package
* **`nonGuiTests`** +
Runs all non-GUI tests in the `seedu.expensetracker`
package
* **`headless`** +
