...
** Sub classes of link:{repoURL}/src/main/java/seedu/url/ui/UiPart.java[`UiPart`] (e.g. `WishListPanel` ) act as 'Controllers', each controlling some part of the UI and communicating with the 'Model' (via the `Logic` component which sits between the 'Controller' and the 'Model').
