...
an `AttributePredicate` is created with the operator (‘<', ‘>’, '< =' or '> =' or '==') and the `NumericAttribute` they are checking for (ie
`Maintenance` or `WaitTime`). The ``AttributePredicate``s are added to a list. The user can add any amount of predicates that
