...
`StatusBarFooter`, `BrowserPanel` etc.
...
`listday`/`listweek` mechanism utilise the same implementation used by the
`list` command:

* `Model#updateFilteredTaskList()` - Take in a predicate parameter and update the model
...
When `listday` command was executed, `DateSamePredicate` will use the current
system date as a 'filter'. `model#updateFilteredTaskList()` which uses `DateSamePredicate`
will update the list of tasks with deadline that match the system date.

Whereas for `listweek` command, `DateWeekSamePredicate` will use all dates from current system date
until the closest Sunday as the 'filter'. `model#updateFilteredTaskList()` which uses `DateWeekSamePredicate`
will update the list of tasks with deadline that matches any of those dates.

As both `listday`/`listweek` are similar, we would only illustrate how `listweek` works.
...
Step 2. `ListWeekCommand#appendDateList(datelist, numDaysTillSunday(dateName))` will generate the `dateList` in
`ddmmyy` format after retrieving the result from `numDaysTillSunday()` method.
...
In order to enable users to better monitor their tasks in the schedule planner, we added `firstday` command.

The `firstday` command will generate the academic calendar weeks and whenever the user launch the application within
either academic week, description for the particular week will append to the application title to allow user
to know the current academic week.
...
* `FirstDayCommand#retrieveWeekDescription(rangeOfWeek)` -- Return the description of a particular week

* `FirstDayCommand#saveRangeOfWeeks(rangeOfWeek)` -- Save the academic calendar weeks data into `rangeOfWeek.xml`

* `FirstDayCommand#retrieveRangeOfWeeks(storeRangeOfWeeks)` -- Retrieved the saved academic calendar weeks data from
`rangeOfWeek.xml`

* `FirstDayCommand#computeRangeOfWeek(firstDay)` -- Generate the academic calendar weeks data based on the `firstDay`
 parameter.

* `FirstDayCommand#addDescription` -- Append description for each academic calendar weeks.

* `FirstDayCommand#isMonday(inputDate)` -- Check if inputDate is Monday and return true or false.
...
Step 3. Methods `onlyOneSetArgument`, `Date.isValidDate` and `isMonday` are to be used to check if `trimmedArgs` is
valid.
...
Step 9. When user launch the application, `MainApp` will check if `rangeOfWeek.xml` exists. Should it be the first
time user launch the application or if user deleted the file, it will create a default file that would not affect the
 user experiences.

If the file exist, `FirstDayCommand#retrieveRangeOfWeeks` will be called to retrieve the academic
 calendar weeks data from `rangeOfWeek.xml`. `FirstDayCommand#isWithinDateRange` would then be called to check if
 current system date lies within the academic calendar weeks and update the application title correspondly if necessary.
...
Step 3. `FilterCommand` is then created, which contains the `TagsContainsKeywordsPredicate`. `FilterCommand.execute()
` returns a list of tasks containing any of the tags input by the user.
...
1.  User request to key in a specific date to generate the academic calendar weeks.
2.  Student Schedule Planner generate the academic calendar weeks.
3.  Student Schedule Planner save the academic calendar weeks.
4.  Student Schedule Planner display to user the command result.
5.  User launch the Student Schedule Planner at any time after Step 1 - 4.
6.  Student Schedule Planner retrieve data saved.
7.  Student Schedule Planner display the updated application title based on the retrieved data.
