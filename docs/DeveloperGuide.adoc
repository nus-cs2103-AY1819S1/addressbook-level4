...
Upon startup, notification preference is loaded from preferences.json and saved to the model.
...
* `logic#parser#NotificationCommandParser()` -- creates a the relevant NotificationCommand object based on user's input
* `logic#commands#NotificationCommand()` -- updates the model's notification preference
...
===== Aspect: How notification preferences updated
