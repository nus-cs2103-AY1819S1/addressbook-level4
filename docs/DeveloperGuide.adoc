...
range of index. The `DiscountItemCommand` object create another Item with the discounted price and indirectly call
`UniqueItemList#setItem(Item target, Item editedItem)` to give the discount the `Item`. It will then save the current
 state for undo/redo.
