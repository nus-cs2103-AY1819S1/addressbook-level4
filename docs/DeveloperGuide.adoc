...
. The user executes the `login` command by specifying the role, name and password. The `login` command determine the type of role that the user entered.
...
.. The `login` command verify password using `HashUtil#verifyPassword()` with the given doctor's password and password entered by user.
...
.. the `login` command call `Model#hasReceptionist()` to ensure that this receptionist exist inside ClinicIO.
.. Once ClinicIO check if user exists, the `login` command call `Model#getReceptionist()` to retrieve the receptionist found in `UniqueReceptionistList`.
.. The `login` command verify password using `HashUtil#verifyPassword()` with the given receptionist's password and password entered by user.
...
An alternative is to open another alert window to prompt user for password.

* Pros: More secure as the entire password will be censored. 
 
* Cons: Require a new window to be opened. 
This will consume time depending on the system. 
In addition, This will take away the user experience. 
