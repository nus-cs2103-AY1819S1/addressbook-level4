...
[TIP]
A new registered `Student` would be initialized and automatically logged in which enabling the student to perform save command. +
...
The data security feature involves mainly the encryption of the user data file when using the `save` command and the decryption of a user user data when logging in using the `login` command. +

[NOTE]
Advanced Encryption Standard (AES) will be algorithm used for encryption.

This section will describe in detail the current implementation and the design considerations of the data encryption & decryption feature.
...
The `Hashing` class is used to generate SHA-1 hash to ensure that there are at least 16 bytes (128 bits) which is required to generate `SecretKeySpec`.
...
* **Alternative 1 (current choice):** Implement as in-built feature.
...
* **Alternative 1 (current choice):** Generate key using a user's password.
** Pros: User do not need to supply additional information or file for encryption/decryption.
** Cons: If a user forgets their password, they are unable to retrieve their user data file.
* **Alternative 2:** Generate key using `KeyGenerator` and an additional file is needed to login.
** Pros: The key will be more secured compared to the key generated from alternative 1.
** Cons: User is unable to decrypt if the key file is lost or corrupted.
// end::dataencryption[]
