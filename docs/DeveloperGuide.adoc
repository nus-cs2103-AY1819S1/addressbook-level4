...
in EventsPlus+'s addressbook). Following this, the appropriate person is obtained from the `FilteredPersonList` (note: this is so as to allow
...
For simplicity, only the logic and event handling sections of the sequence diagram are shown below for generateLocation command:
