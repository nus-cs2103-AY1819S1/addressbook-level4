...
Users can generate a `MedicalCertificate` for a specific `ServedPatient` by invoking the `ReferralLetterCommand` accompanied by the served patient's index in the `ServedPatientList`. +
