...
image::RegisterCommandUML.png[width="200"]
...
Figure 2 below shows the sequence diagram of the `RegisterCommandParser`.

image::SDforRegisterCommandParser.png[width="800"]
_Figure 2. Sequence Diagram for the parsing of `RegisterCommandParser`._

Figure 3 below shows the high-level sequence diagram of the command execution.

image::SDRegister.png[width="800"]
...
image::LoginCommandUML.png[width="200"]
...
Figure 2 below shows the sequence diagram of the `LoginCommandParser`.

image::SDforLoginCommandParser.png[width="800"]
_Figure 2. Sequence Diagram for the parsing of `LoginCommandParser`._
...
1. The user launches the application. As explained earlier, since the `currentUser` is initialized as `null`,
...
4. Should the credential not be valid or does not match an existing credential in the `CredentialStore`, the user will simply be shown an failure message.
...
** Cons: Additional memory resources is used to store the data structures involved to
