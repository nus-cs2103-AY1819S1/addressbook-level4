...
As of v1.2.1, the storage, commands, and parsers are distinct classes for each type of entity.
...
The UI consists of three windows: The `PersonWindow`, `ModuleWindow` and `OccasionWindow`. Each of these windows inherits from the `MainWindow`.
...
=== [Proposed] Features supporting Person, Module and Occasion
Our application is an extension of address book application. Besides
supporting basic address book features for 'Person', we also intended to
make those features be available for 'Module' and 'Occasion'. This makes this
application(TheTracker) to be able to track the modules a person takes, as
well as the Occasions they participate in.

==== Current Design plan
The graph below shows the inheritance relationship between the related
interface and classes.

image::ClassInheritance.png[width="800"]

We would be using polymorphism to realise support for 'Person', 'Module' and
'Occasion'.
...
David in his TheTracker. The 'find' command calls 'Model#commitAddressBook()
', causing the modified state of TheTracker after the 'find David...' command
...
// end::autocomplete[]
>>>>>>> 1047a3c7e9ee7a6c6ba36efd18eaee63c15a5e62
...
It supports deletions of `Person`, `Module` and `Occasion', by the same `DeleteCommand` with different parameters, which would facilitate
and improve the user experience.

==== Current Implementation
The `delete` mechanism is managed by `DeleteCommand`, which extends the `Command` class and uses inheritance to facilitate implementation.
...
image::DeleteStartingStateListDiagramXueyan.png[width="800"]

Step 2: The user executes `add n/David...` command to add a person named
David in his TheTracker. The parser `AddCommandParser` will identify by
...
after the `add David...` command executes to be saved in the `addressBookStateList`.

image::DeleteNewCommand1StateListDiagramXueyan.png[width="800"]

Step 3: The user executes `delete n/David` to delete the person named David in the address
book. The parser `DeleteCommandParser` will identify by the '\n' tag that the entity to be
deleted is a person.The `delete` command calls Model#getFilteredEntity(), Model#deleteEntity()
and 'Model#commitAddressBook()', and causes modified address book state to be saved into the 'addressBookStateList'

image::DeleteDeleteCommandStateListDiagramXueyan.png[width="800"]


The following sequence diagram shows how the delete operation works:

image::DeleteSequenceDiagramXueyan.png[width="800"]

==== Design Considerations
===== Aspect: How delete executes

* **Alternative 1 (Use tags and entity):** Inputs the type of the entity to be deleted.
** Pros:
