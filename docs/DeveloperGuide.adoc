...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/address/model/patient/patient.java[`patient`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] allows you to update a patient, and link:{repoURL}/src/main/java/seedu/address/model/patient/patient.java[`patient`] allows you to update the tags.
* Solution
...
****
* Hints
** The tag labels are created inside link:{repoURL}/src/main/java/seedu/address/ui/patientCard.java[the `patientCard` constructor] (`new Label(tag.tagName)`). https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Label.html[JavaFX's `Label` class] allows you to modify the style of each Label, such as changing its color.
** Use the .css attribute `-fx-background-color` to add a color.
** You may wish to modify link:{repoURL}/src/main/resources/view/DarkTheme.css[`DarkTheme.css`] to include some pre-defined colors using css, especially if you have experience with web-based css.
* Solution
** You can modify the existing test methods for `patientCard` 's to include testing the tag's color as well.
...
===== [Step 3] Ui: Add a placeholder for remark in `patientCard`
Let's add a placeholder on all our link:{repoURL}/src/main/java/seedu/address/ui/patientCard.java[`patientCard`] s to display a remark for each patient later.

**Main:**

. Add a `Label` with any random text inside link:{repoURL}/src/main/resources/view/patientListCard.fxml[`patientListCard.fxml`].
. Add FXML annotation in link:{repoURL}/src/main/java/seedu/address/ui/patientCard.java[`patientCard`] to tie the variable to the actual label.

**Tests:**

. Modify link:{repoURL}/src/test/java/guitests/guihandles/patientCardHandle.java[`patientCardHandle`] so that future tests can read the contents of the remark label.

===== [Step 4] Model: Add `Remark` class
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/address/model/patient/patient.java[`patient`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.
...
===== [Step 5] Model: Modify `patient` to support a `Remark` field
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/address/model/patient/patient.java[`patient`].

**Main:**

. Add `getRemark()` in link:{repoURL}/src/main/java/seedu/address/model/patient/patient.java[`patient`].
...
===== [Step 6] Storage: Add `Remark` field to `XmlAdaptedpatient` class
We now have `Remark` s for `patient` s, but they will be gone when we exit the application. Let's modify link:{repoURL}/src/main/java/seedu/address/storage/XmlAdaptedpatient.java[`XmlAdaptedpatient`] to include a `Remark` field so that it will be saved.
...
. Fix `invalidAndValidpatientAddressBook.xml`, `typicalpatientsAddressBook.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.

===== [Step 6b] Test: Add withRemark() for `patientBuilder`
Since `patient` can now have a `Remark`, we should add a helper method to link:{repoURL}/src/test/java/seedu/address/testutil/patientBuilder.java[`patientBuilder`], so that users are able to create remarks when building a link:{repoURL}/src/main/java/seedu/address/model/patient/patient.java[`patient`].

**Tests:**

. Add a new method `withRemark()` for link:{repoURL}/src/test/java/seedu/address/testutil/patientBuilder.java[`patientBuilder`]. This method will create a new `Remark` for the patient that it is currently building.
. Try and use the method on any sample `patient` in link:{repoURL}/src/test/java/seedu/address/testutil/Typicalpatients.java[`Typicalpatients`].

===== [Step 7] Ui: Connect `Remark` field to `patientCard`
Our remark label in link:{repoURL}/src/main/java/seedu/address/ui/patientCard.java[`patientCard`] is still a placeholder. Let's bring it to life by binding it with the actual `remark` field.

**Main:**

. Modify link:{repoURL}/src/main/java/seedu/address/ui/patientCard.java[`patientCard`]'s constructor to bind the `Remark` field to the `patient` 's remark.

**Tests:**

. Modify link:{repoURL}/src/test/java/seedu/address/ui/testutil/GuiTestAssert.java[`GuiTestAssert#assertCardDisplayspatient(...)`] so that it will compare the now-functioning remark label.
