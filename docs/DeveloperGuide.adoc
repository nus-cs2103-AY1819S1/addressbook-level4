...
|===
|Appointment type |Procedure name |Date and time |Doctor-in-charge
...
** Pros: Allows for easier handling of incorrect values
** Cons: More effort needed to filter and retrieve the different types
...
image::AddmhViewmhSequenceDiagram.png[width="1050"]
...
** This class is a wrapper class around the internal representation of a `Set` of `Diet`.
...
* **Alternative 1 (current choice):** Use a `Enum` inside the `Diet class` and contain all `Diet` in one collection.
** Pros: Less repetitive code and cleaner design.
** Cons: More efforts are needed to filter or retrieve different types of `Diet` from one `DietCollection`.

* **Alternative 2:** Use polymorphism to extends `Diet` class and add three different collections to a `Person`.
** Pros: Easier to retrieve different types of dietary requirements.
** Cons: A lot of repetitive codes since the three different types do not differ much.

===== Aspect: Data structure to hold the different `Diet` objects

* **Alternative 1 (current choice):** Use `HashSet` and override the `hashCode` for `Diet`.
** Pros: Easier to handle duplication in adding dietary requirement.
** Cons: The sequence of adding dietary requirements is lost. (However, the sequence is not important for the current set of features implemented.)

* **Alternative 2:** Use `ArrayList`.
** Pros: The sequence of adding the dietary requirements is preserved.
** Cons: Harder to handle duplicates.
