...
This guide assume that readers has certain developers knowledge.
...
The `LogoutCommand` will post an event to notify the UI that you want to log out of ClinicIO.

Here is a step-by-step on how `LogoutCommand` notify `MainWindow` that you want to log out of ClinicIo: +

Step 1: The `LogoutCommand` will post a new `LogoutClinicIoEvent` to the `EventsCenter`.

Step 2: The `eventBus` will then post the newly created `LogoutClinicIoEvent` (from Step 1).
