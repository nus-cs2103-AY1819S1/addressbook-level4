...
. Upon first running the app, the corresponding zip package containing ImageMagick will be copied out of the jar and unzipped. A temporary folder for ImageMagick output is also created.

. The user opens an image with the `open` command. The method `model.getPreviewImage()` will be called to read the appropriate `BufferedImage` from disk.

. The user enters the command (e.g. `apply blur 0x8`). The entered command is parsed and the corresponding transformation is created.
...
[start=4]
. `ImageMagick#processImage` is a quasi-factory method that creates an instance of `ProcessBuilder` which contains the all the necessary information for ImageMagick to apply a transformation to the image.
. The same method also executes the `ProcessBuilder` and reads the output back from the temporary folder.
[NOTE]
...
[start=6]
. If successful, the transformation that is just executed will be added to the `PreviewImage`++'s++ `TransformationSet`.

. The command will then request the UI to update its components, namely the `HistoryListPanel` and `ImagePanel` if any transformation was applied.
...
.. Other incorrect cd commands to try: `open 11` (as index exceeds batch size of 10), `open x` (where x exceeds the total images in the current batch),
   `open abc` +
   Expected: Similar to previous.
