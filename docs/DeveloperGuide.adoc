...
`ModelManager#removeTagsFromPersons(ArrayList<Person> target, ArrayList<Person> original)` is then invoked to update
the internal list with the new persons list.

*Step 4:* After the internal list is updated, `ModelManager#removeTagsFromPersons(ArrayList<Person> target, ArrayList<Person> original)`
then invokes `ModelManager#indicateAddressBookChanged()`, which raises an `AddressBookChangedEvent` which is handled by
`EventsCenter`.
...
=== Design Considerations
==== Aspect: Identifying persons associated with CCAs to be erased
...
* **Alternative 2:** Store persons in a multilayer linked list.
