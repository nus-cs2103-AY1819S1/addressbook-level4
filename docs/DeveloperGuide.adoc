 ...
 * `Model#getDirectoryImageList` -- Retrieved the stored list of images in UserPrefs.
...
The Model calls upon `UserPrefs` which stores and facilitates updates and retrieval of the `imageList` in the current
...
Other operation, `Model#updateCurrentOriginalImage(Image, Path)` stores the path of the selected image
as `currOriginalImage` and the `PreviewImage` instance of the path as `currentPreviewImage` within the `ModelManager`.
...
Step 2. The user executes `cd Desktop` command to navigate into the Desktop directory. The cd command calls
`Model#updateImageList()` to retrieve all the images within Desktop.
...
** Pros: Work on a small size of images.
...
* *Alternative 1 (current choice)*: Removes and updates `imageList` in `UserPrefs`.
...
** Cons: Harder to implement and keep track of the the current start pointer.
