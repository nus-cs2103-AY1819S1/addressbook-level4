...
respective stock up amounts. With this HashMap, it creates a `StockUpCommand` object. The `StockUpCommand` executes
the stock up operation, which is exposed in the `Model` interface as `Model#stockUpIngredients()`. The operation is
atomic. This means that if the operation fails for one ingredient, none of the ingredients will be stocked up.
...
** Cons: Difficult to integrate different features while maintaining abstraction. The <<auto-ingredient-update>>
feature would require the sales component to create each updated ingredient, before calling the `updateIngredient()`
