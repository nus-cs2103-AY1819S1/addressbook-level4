...
.. Test case: Open up the student file and remove the contents in the <code> tag from <moduleStaged> (e.g. instead of <code>CS1231</code> it should look like <code></code> after editing). Now login as the student using the updated user data file and issue the generate command. +
...
.. Other invalid modification to try: removing the entire <code> tag from a module in <modulesStaged> or <modulesTaken>,  . +
Expected: Similar to previous.
