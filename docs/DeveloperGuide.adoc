...
// tag::select[]
=== Select feature
`SelectCommand` allow users to select from the currently viewed batch of 10 images within the directory for image-editing.
This command is further facilitated by the `NextCommand`.

==== Current Implementation
The implementation of the Select feature is largely similar to `CdCommand`. It is facilitated by the following operations in `ModelManager`:
...
Given below is an example usage scenario of how the select and next mechanism behaves at each step.
...
. The user executes `select 1` command to select the first image in the first batch of 10 images. The select
...
. The user then executes `select 5` command to select the fifth image in the batch of 10 images. The fifth image
...
.Sequence Diagram for SelectCommand
...
image::ApplyCommand.png[width="800"]
...
<<<<<<< HEAD
 - As in the real life, combination of the transformations are commonly needed, thus we build create command
