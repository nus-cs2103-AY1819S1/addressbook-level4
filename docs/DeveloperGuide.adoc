...
Step 3. The validated new command will be stored in the json for in the PicosonCommands folder create when first time run the app.
...
There are 5 main google-related commands, with the first two commands to login/logout a user, and the latter 3 being overloaded command types with `GoogleCommand` as the parent class.
...
WARNING: Currently, the user MUST go through with the login process once activated, else Piconso will freeze. Suggestions are to implement asynchronous login
