...
A `timing` attribute has been added into the list of attributes of Person class.
...
===== Aspect: Data structure to support the timing attribute
...
Since the `timing` attribute of a person is an array list, we created a new command to add `Time` for a student, called `AddTimeCommand`.
...
* After inputing the command, the inputs will be taken into `AddressBookParser` class and an `AddTimeCommand` object will be returned. The command will then be executed and `Time` will be added for the students at `Index`.

* During the execution of the command, a `Time` object will be created. It will then search through Tutor Pal to see if the `timing` of student at `Index` contains the `Time`. If it does, it will then check if there are any clashes with the timings already in Tutor Pal. If there are no errors, `Time` will be added into his `timing`, otherwise, an error message, depending on the type of error, will be shown.
