...
* `DeletePersonCommand#excute(Model model, CommandHistory history)`:
deletes the person passed to the `DeletePersonCommand` class

This operation is exposed in the Model interface as Model#getFilteredEntity(), Model#deleteEntity() and
Model#commitAddressBook() respectively.
