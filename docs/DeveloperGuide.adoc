...
. Open link:{repoURL}/blob/master/src/main/java/seedu/scheduler/storage/XmlAdaptedEvent.java[`XmlAdaptedEvent.java`] and link:{repoURL}/blob/master/src/main/java/seedu/scheduler/ui/MainWindow.java[`MainWindow.java`] and check for any code errors
...
**Step 5** For each event extracted, a [proposed] eventConvertor utility method called `addGcEventToLocal` is called to convert the Google Extracted Event to local Event.
...
v1.1 Partial impementaion on GetGCEvents
v1.2 Implementation on GetGCEvents
v1.3 Implementation of all relevant commands to sycn with Google Calander in real time
v1.4 Upgrade implementation to synchronous version
