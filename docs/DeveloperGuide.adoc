...
. The `ShowStatsRequestEvent` event is handled by `MainWindow#handleShowStatsEvent()`, which creates a new `StatsWindow` object after getting the
...
. The `StatsWindow` object gets created and calls `StatsWindow#setData()`, which creates a `BarChart` with the expense data received, which is then added to the `ChartArea` pane.

. This results in a new window showing a bar chart of expenses over the last 7 days.
...
*	BudgetPanel#budgetBar – A progress bar element that visually presents the percentage of the current budget cap that has been used.

Given below is an example usage scenario and how the budget panel behaves at each step:

Initial State. The user launches the application and signs up for a new account. The MainWindow creates a new 'BudgetPanel', which elements are initialized as follows:
...
Step 1: The user executes the command add n/Lunch $/30.00 c/Food. As the 'add' command modifies the budget and expenses, 'AddCommand#execute()'' will post a 'UpdateBudgetPanelEvent' event to the EventsCenter.
...
*Step 4*
Two 'KeyFrame' objects are added to the Timeline that smooths the transition of the 'BudgetPanel#budgetBar' progress percentage from 0% to (4.0/28.0)%
[Note]
...
*Step 5*
A call to 'BudgetPanel#incrementKeyFrames()' is made to create the incrementing animation for 'BudgetPanel#expenseDisplay'. The total time for this animation is 0.5 seconds, and consists of 10 'KeyFrame's.
With this information, both the amount of time between 'KeyFrame's and the amount to increment 'BudgetPanel#expenseDisplay' by each 'KeyFrame' can be calculated. 'BudgetPanel#updateExpenseDisplay()' is called to
increment the 'BudgetPanel#expenseDisplay'.
[Note]
...
*Step 5*
BudgetPanel#update()also calls BudgetPanel#BudgetPanel#setBudgetUiColors(). The method detects that the user has gone over budget, and changes the color of 'BudgetPanel#expenseDisplay' and 'BudgetPanel#budgetBar' from green to red.
...
. Fix `invalidAndValidExpenseAddressBook.xml`, `typicalPersonsAddressBook.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
