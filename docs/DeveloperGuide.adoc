...
. The user then executes `open 5` command to open the fifth image in the batch of 10 images. The fifth image
...
. The user opens an image with the `open` command. The `Canvas` is initialized with a new `Layer` which initializes its own `PreviewImage` with the opened image. The `currentStatePointer` pointing to that state.
...
. The user opens an image with the `open` command. the method `model.getPreviewImage()` will be called to get the bufferedImage of the current preview scene.
...
. parse the image name user enters, get the name and the format of the filename.

. check the format of the image, if the format is not supported, error will be thrown
