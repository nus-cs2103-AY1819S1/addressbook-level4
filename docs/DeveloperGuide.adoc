...
. Once the email is saved in the `EmailModel`, the `ModelManager` raises an `EmailSavedEvent`, to indicate that a new
 email is saved to the `EmailModel`.

. The `EmailSavedEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleEmailSavedEvent
...
. `create` command will check for existing CCA name using `BudgetBook#hasCca(Cca toAdd)`. If a CCA with the same
name exists, the CCA will not be created. Otherwise, it will be added into the `BudgetBook` in the `Model`.

. `BudgetBook#addCca(Cca cca)` will invoke `ModelManger#indicateBudgetBookChange()` to raise a
...
=== Site-wide documentation dettings
