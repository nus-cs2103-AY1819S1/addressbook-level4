...
|`* * *` |user who wants to track their spending |add a new expenseTemp |have the app keep track of my expenses
...
2. Once deleted successfully, expenseTemp tracker shows a success message.
...
1. User requests to log in to expenseTemp tracker
...
1. User requests to logout of the expenseTemp tracker
...
*MSS*
1. User key in command to set recurring expenses for an expenseTemp that was already keyed in
2. Expense tracker prompts the user to key in which expenseTemp and the frequency of recurrence.
3. User keys in the index of the expenditure to set as recurring expenseTemp and the frequency
4. Expense tracker adds the expenseTemp automatically every time the set amount of time passes.
+
Use case ends
