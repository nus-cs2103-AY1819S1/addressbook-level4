...
image::Template_SD1.png[width="600"]
...
image::Template_SD2.png[width="600"]
...
.Workflow for a user using the Awareness features
image::awarenessFlow.png[width=250, height=250]

The diagram shows that there are 2 functions provided as part of the Contextual Awareness feature.
...
.Sequence diagram showing how Awareness finds a Matching ResumeEntry
image::awarenessHighLevelSeq.png[width=400, height=350]

As seen, `ContextCommand` is responsible for (indirectly) calling Awareness. Note that `ContextCommand` leverages on the existing `AddEntryCommand` to add the Matching ResumeEntry to the EntryBook. +
