...
parameters are `person`, `meeting` and `group`, or their shorthand equivalent `p`, `m` and `g`. `ListCommandParser`
...
* Step 2.1. If the parameter is `meeting` / `m`, `Model#updateFilteredMeetingList` is called with
`PREDICATE_SHOW_ALL_MEETINGS` to remove any filters applied to `UniqueMeetingList`.

* Step 2.1. If the parameter is `person` / `p`, `Model#updateFilteredPersonList` is called with
...
image::xmlAdaptedMarshalDiagram.png[]
...
.Class diagram of XmlAdapted classes and their associations
image::xmlAdaptedUnmarshalDiagram.png[]
...
The code snippet below showcase how MeetingBook toggle between the behaviour.
[source, java]
----
...
==== Handling of irrelevant imported data

The behaviour when importing data that is incorrectly formatted differs on where the incorrect data detected. If the data imported is wrong in entirety, `DataConversionException` is thrown and the error message, "Data file provided is not in the correct format." is shown to the user.

However, when a XML file which is valid yet incorrectly formatted (e.g. `persons` stored in `groups` is not stored also in `persons`), the `persons` inside the `members` tag of the XML file is ignored. This removes and prevents the addition of the association between `Person` and `Group` to the current MeetingBook.

Also, when `--force` is not used in the command and integrating the imported groups into the current MeetingBook, the `persons` inside the `members` tag may not be available in the current MeetingBook. The behaviour to handle this situation is the same as how it handles valid yet incorrectly formatted XML.

.Example of correctly Formatted XML file.
[source, xml]
----
...
----
<1> The `Person` is available as it is also contained as `persons` in this XML file  as well as in `members`.

.Example of Incorrectly Formatted XML file where Unknown Person contained inside Group
[source, xml]
----
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<meetingbook>
    ...
    <groups>
        ...
        <members>
            <name>Incorrect User</name> <!--1-->
            // Person info
        </members>
    </groups>
</meetingbook>
----
<1> The `Person` is not available as is not contained as `persons` in this XML file but only in `members`.
...
If we were to overwrite conflicting entries, we lose information in our current MeetingBook as they are overwritten by the imported data.
