...
image::ArchitectureDiagramWithGoogle.png[width="600"]
...
* <<Design-Calendar,*`Calendar`*>>: Google Calendar integration for App.
...
Patient model contains a List of upcoming appointment and List of past appointments.
...
* Patient and Doctor must be mutually exclusive. Patient cannot be a instance of Doctor. This will result in
duplicate person exception
* Patient is identified as a duplicate person if both their name and their number is the same. This allows patient with
the same name or same number to exist.
====

==== Design Considerations
===== Aspect: Data Structure of Upcoming and Past Appointments
* Alternative 1 (Current Choice): Upcoming Appointments stored within a List and Past Appointments stored within an
List. This allows easy traversal and iteration of the appointments. This is also to allow the data structure to match with the
 model and storage component more easily.

* Alternative 2 (Previous Implementation): Upcoming Appointments stored within a PriorityQueue and Past Appointments stored within a
ArrayList. This allows the appointments to be added in a chronological order but it resulted in several instances of
data required to be converted when added to storage.
// end::registerpatient[]
...
* Patient and Doctor must be mutually exclusive. Doctor cannot be a instance of Patient. This will result in
duplicate person exception.
* Doctor is identified as a duplicate person if both their name and their number is the same. This allows doctor with
the same name or same number to exist.
====
...
unsuccessful. Due to the integration of Google OAuth, it is mandatory for user to login before any process can
continue. This is mandatory as our features is complimentary for our application as HealthBook will work at its best with
the google calendar login.
** Pros: Easy to implement
** Cons: Program is unable to continue if google login is not completed.

* Alternative 2 (Alternative Choice): Allow doctor to still be created with or without the Google Login. We implemented
register-doctor to have Google Login as optional.
** Pros: Flexible access to Google Calendar
** Cons: Hard to implement. Have to recreate an Google OAuth system that does not require the program to wait for
verification code.
// end::registerdoctor[]
...
=== Remark Feature
==== Current Implementation
Remark command allows user to input any information into Patient object with no restrictions. This command can
only be called after patient is created.
...
. Fix `invalidAndValidPersonAddressBook.xml`, `typicalPersonsHealthBook.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
