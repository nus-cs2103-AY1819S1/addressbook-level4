...
1. The user launches the application for the first time and the `currentUser` will at this point be `null`.
Issuing the save command at this point will throw an error message indicating to the user that they have not registered an account or they are not logged in.

2a. The user executes `register user/demo ...` to register a new `Student` account.
The `save` command will call `Model#saveUserFile(...)`. +

2b. The user executes `login user/demo pass/#Qwerty1 userData/userdata.xml` to log into account.
    The `save` command will call `Model#saveUserFile(...)`. +
