...
[NOTE]
As a more OOP model, we can store a `Tag` list in `MeetingBook`, which `Person` can reference.
This would allow `MeetingBook` to only require one `Tag` object per unique `Tag`, instead of each `Person` needing their own `Tag` object.
...
* `versionedAddressBook` now also contains group list, which supports categorisation of persons in MeetingBook.
