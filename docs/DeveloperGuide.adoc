...
After adding a saving, the FilteredWishList is also updated to reflect the latest WishBook.

Given below is an example usage scenario and how the SaveCommand behaves at each step:

Step 1. The user launches the application for the first time.
WishBook will be initialised in `ModelManager` with the initial WishBook state, where `SavedAmount spareFunds` = 0.0 .

Step 2. The user executes `save 1 s/10`, to save $10 into an existing wish with `Index` 1 and `Price` $15. In the SaveCommand#execute method,
a new editedWish is created to reflect the new savedAmount. It then calls Model#updateWish to update this wish with the old one in `WishBook`.
...
[NOTE]
If the `Index` is invalid, or the saving entered causes `savedAmount` to become negative, it will not call Model#updateWish.

Step 3. The user decides to execute save 1 s/10 again. However, SaveCommand checks that `savedAmount` > `price`.
SaveCommand#execute creates a new `editedWish` with `savedAmount = wish.getPrice()`.
