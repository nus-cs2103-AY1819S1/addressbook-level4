...
===== Aspect: Logic
Commands will be required to create a `requiredPermission:PermissionSet` object, and populate it with all `Permission` the command requires user to have to execute the command. 

Given below is an example scenario of how the every commands will be executed.
...
** Cons: Need to ensure that there is at least 1 user that can assign permissions to other users. 
