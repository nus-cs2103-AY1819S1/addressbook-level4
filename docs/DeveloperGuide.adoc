...
* **Alternative 1 (current choice):** Re-assign Model's favouriteEvet regardless of previous preference
...
Step 6. The old event is replaced with the new one in the system from the `Model` component, through the execution of `Model#updateEvent`.
...
The following sequence diagram shows how the readImportContactsFile operatoin works:
