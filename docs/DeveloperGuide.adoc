...
=== View a range of tasks

In order to enable users to better manage their tasks in *Student Schedule Planner*, we added two more variations of
 the `list` command. The `listday` command supports viewing tasks due the current date whereas `listweek` command
 supports viewing tasks from the current date until the nearest Sunday.

==== Implementation

The details of the implementation are broken down into several sections below.

==== Date computation

When the `listday` command was used, the current system date will be retrieved. Whereas for the `listweek` command,
the range of date from current system date until Sunday will be computed and retrieved.

==== Data retrieval

Based on the range of date given, the *Student Schedule Planner* will retrieve all the tasks that matches the date
that was computed and display to the screen.
