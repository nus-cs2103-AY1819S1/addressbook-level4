...
Step 5. `AddEventCommand` calls `Model#addEvent` after all checks have passed. The event is added into the address book from the `Model` component.
...
For instance, when an `Event` is added,
the event triggers are handled as follows.
