...
The potential to evolve this system is endless, from including artificial intelligence
to manage the clinic supply without human intervention to delivering the first-hand information
from medical breakthroughs to treat patients more effectively!

==== Current Implementation
The _Medicine_ class and the properties of the medicine are stored in the Model package.
...
See the User Guide for the list of commands related to the medicine class.

Given below are examples of how the medicine commands are used:

Scenario 1. The clinic orders a new medicine that they do not currently have in their store.
 The user uses the `addMedicine` command to add the new medicine into the records.

Scenario 2. There is a wrong entry of one of the serial number of the medicine. The user types
`editmedicine` to correct the serial number. The code on how it works is shown below:
...
Scenario 4. The user needs to check for the stock level of every medicine.
The user executes the `checkStock` command
which lists the medicine that are below their Minimum Stock Quantity.

.Filter medicines that are low in supply
image::checkstockfilter.png[width="800"]

==== Design Considerations
// preface
The medicines in the clinic were needed to be stored in a Java Collections.
Below were the possible ones we chose and the analysis of each one.

===== Aspect: Execution of the command

* **Alternative 1 (current choice):** Stores the medicine in a list.
** Pros: Many useful methods to manipulate data.
...
** Cons: Many commands need to change to list to use the method and change back, more complex.
//end::medicine[]
