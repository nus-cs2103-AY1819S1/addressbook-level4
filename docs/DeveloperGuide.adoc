...
** Cons: Additional entity `Multiplicity` may have to be frequently edited under each as it is another
field that is affected by multiple commands.
...
** Cons: There might be collisions in `CreatedTime` if the the time stored is incorrect.
...
During the computation of wish expiry date is handled internally in the `ParserUtil` class,
which is able to take days, months, and years as input to generate a proper `Date` with the current time the user executes the command.
Internally, `ParserUtil#parseDate()` converts the input time into number of days, and adds it to the current date to get the desired `Date`.
...
Step 1. The user executes `save 1 s/10`, to save $10 into an existing wish with `Index` 1 and `Price` $15. The $10 is
...
Step 3. The user decides to execute save 1 s/10 again. However, SaveCommand checks that `savedAmount` > `price`.
...
Step 5. The user tries to execute `save 1 s/10` again. However, since the value for `fulfilled` is true,
