...
[NOTE]
The commands pertaining to module database admnistration can only be executed by users of type `Admin`.
...
* `ModuleList#hasModule(...)` -- This method check if the given module already exist.
...
+
[NOTE]
We can fix this by having different implementations
for the module list but this can be difficult to implement.
