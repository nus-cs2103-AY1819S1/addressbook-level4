...
The primary goal of this section is to explain the implementation of the
login/logout feature and the changes that were made on other components of Event Organiser,
namely, the EditUser Command, and the DeleteUser Command, to ensure cohesiveness of the
entire application.

image::LoginFeature_SequenceDiagram_includes_Logic.png[width="800"]

image::LoginFeature_SequenceDiagram_includes_Model.png[width="800"]

When the execute() method of the login command is called,  the login command authenticates
the user i.e. checks that the user that is trying to logging actually exists in Event Organiser.
...
On the other hand, when the execute() method of the log out command is called,
...
For example, if the user, Alex Yeoh, wants to login to his account, the following image is
what he will see before he logs in.

image::LoginFeatureBeforeLogin.png[width="300"]

Once Alex Yeoh successfully logs in, the UI will automatically be updated. A successful
log in is indicated by the change in colour of the user's name from white to green
as shown in the image below. The user's name will remain in green as long as he is logged in.
After he logs out,the colour of his name will revert back to white as shown in the image above.
...
image::LoginFeature_ClassDiagram_Model_2.png[width="800", margin-top="0"]
...
For
users must be logged in to edit or delete his/her user account.
