...
image::SaveCommandUML.png[width="350"]
_Figure 1. SaveCommand UML Diagram_

The `Save` command uses multiple components of the modsUni application.
...
The `SaveUserChangeEvent` mentioned above is handled by the Storage component as show in Figure 3.

image::SequenceDiagramforSaveEventHandling.png[width="650"]
_Figure 3. High Level Sequence Diagram showing how the Storage component handles the `SaveUserChangedEvent`_

Storage makes use of a `UserStorage` object to write `User` to the file specified by the filePath.
...
1. The user launches the application for the first time. The `currentUser` will at this point be `null`.
This would prevent any commands aside from `register` and `login` to be available to the user. +

2. The user executes `register user/demo ...` to register a new `Student` account.
