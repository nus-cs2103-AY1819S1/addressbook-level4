...
image::AddStartingStateListDiagram.png[width="800"]
...
image::AddNewCommand1StateListDiagram.png[width="800"]

The following sequence diagram shows how the `addperson` operation works:
`addperson n/David`

image::AddCommandLogicDiagram.png[width="800"]
...
* **Alternative 1 (Current Implementation):** `EditPersonCommand`, `EditModuleCommand` and `EditOccasionCommand` are totally irrelevant to each other
**Pros:
...
image::EditXmlCommandSequenceDiagram.png[width="800"]
...
// tag::delete[]
=== Delete
`delete` is a basic feature provided by `TheTracker`.
...
follows the same mechanism. `DeletePersonCommand` extends the `Command` class and uses inheritance to facilitate
implementation.

Generally, it implements the following operation:

* `DeletePersonCommand#excute(Model model, CommandHistory history)`: deletes the Person of the `Index` passed to the `DeletePersonCommand` class
...
David in his TheTracker. The parser `AddPersonCommandParser` will parse the person's attributes and create
a new person object with these attributes and pass the person to `AddPersonCommand`. The `addperson` command
calls `Model#commitAddressBook()`, causing the modified state of TheTracker
after the `addperson n/ David...` command executes to be saved in the `addressBookStateList`.
(refer to the Add Command diagram)

Step 3: The user executes `deleteperson 1` to delete the person who has an index 1 (named David) in the current active list.
The parser `DeletePersonCommandParser` will identify by the index of the person to be
