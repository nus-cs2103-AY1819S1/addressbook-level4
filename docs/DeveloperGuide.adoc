...
=== Budget
==== Current Implementation
The budget feature is one whereby after a user sets a budget, if the total spendings of a user exceeds his budget
during a period determined by the user, a warning will show up to inform the user that adding an expense results in
him exceeding his set budget.

The current implementation for budget tracking is described as below.

===== Setting a budget
Below would be how a budget is set by the user.
...
method to update the time where the next budget is reset.
