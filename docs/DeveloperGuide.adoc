...
==== Current Implementation
The `ExportPatientData` class is responsible for filtering and organizing the relevant data. It then passes the data to
`ExportUtil` in order to write to a CSV file. The names and locations of the CSV files are predetermined by
`ExportPatientData`.

The following operations are implemented: +

* `exportpatients` Command - exports all patients' personal information.
* `exportpatientsappointments` Command - exports all patients' appointment records.
* `exportpatientsconsultations` Command - exports all patients' consultations records.

==== `exportpatients` Command

{add implementation details}

==== `exportpatientsappointments` Command

{add implementation details}

==== `exportpatientsconsultations` Command
...
* **Alternative 1 (current choice):** Predetermined within the program to be stored in the root folder.
