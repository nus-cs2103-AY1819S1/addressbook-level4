...
The export mechanism is facilitated by `ExportCommand`. `ExportCommandParser` is responsible for parsing the input arguments. +
[NOTE]
...
//TODO Explain XML
The program will raise a `MeetingBookExportEvent` to `EventCenter`. `StorageManager` which subscribed to the `EventCenter` which will trigger `handleMeetingBookExportEvent` when received `MeetingBookExportEvent`. This method will then export the data as a XML file using `Model#exportMeetingBook`.
...
+
These scenarios create unnecessary complications when we want to attempt undoing the operation. Therefore we do not allow the command to be undoable.
...
** link:{repoURL}/src/main/java/seedu/meeting/logic/parser/AddressBookParser.java[`AddressBookParser`] is responsible for analyzing command words.
* Solution
** Modify the switch statement in link:{repoURL}/src/main/java/seedu/meeting/logic/parser/AddressBookParser.java[`AddressBookParser#parseCommand(String)`] such that both the proper command word and alias can be used to execute the same intended command.
...
****
* Hints
** The link:{repoURL}/src/main/java/seedu/meeting/model/Model.java[`Model`] and the link:{repoURL}/src/main/java/seedu/meeting/model/AddressBook.java[`AddressBook`] API need to be updated.
** Think about how you can use SLAP to design the method. Where should we place the main logic of deleting tags?
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/meeting/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/meeting/model/person/Person.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/meeting/model/AddressBook.java[`AddressBook`] allows you to update a person, and link:{repoURL}/src/main/java/seedu/meeting/model/person/Person.java[`Person`] allows you to update the tags.
* Solution
** Implement a `removeTag(Tag)` method in link:{repoURL}/src/main/java/seedu/meeting/model/AddressBook.java[`AddressBook`]. Loop through each person, and remove the `tag` from each person.
** Add a new API method `deleteTag(Tag)` in link:{repoURL}/src/main/java/seedu/meeting/model/ModelManager.java[`ModelManager`]. Your link:{repoURL}/src/main/java/seedu/meeting/model/ModelManager.java[`ModelManager`] should call `AddressBook#removeTag(Tag)`.
...
** Use link:{repoURL}/src/main/java/seedu/meeting/ui/StatusBarFooter.java[`StatusBarFooter#handleAddressBookChangedEvent(AddressBookChangedEvent)`] to update the number of persons whenever there are new changes to the MeetingBook.
...
****
* Hint
** Add the API method in link:{repoURL}/src/main/java/seedu/meeting/storage/AddressBookStorage.java[`AddressBookStorage`] interface.
** Implement the logic in link:{repoURL}/src/main/java/seedu/meeting/storage/StorageManager.java[`StorageManager`] and link:{repoURL}/src/main/java/seedu/meeting/storage/XmlAddressBookStorage.java[`XmlAddressBookStorage`] class.
* Solution
...
. Modify link:{repoURL}/src/main/java/seedu/meeting/logic/parser/AddressBookParser.java[`AddressBookParser`] to accept a `RemarkCommand`.

**Tests:**

. Add `RemarkCommandTest` that tests that `execute()` throws an Exception.
. Add new test method to link:{repoURL}/src/test/java/seedu/meeting/logic/parser/AddressBookParserTest.java[`AddressBookParserTest`], which tests that typing "remark" returns an instance of `RemarkCommand`.
...
. Modify link:{repoURL}/src/main/java/seedu/meeting/logic/parser/AddressBookParser.java[`AddressBookParser`] to use the newly implemented `RemarkCommandParser`.
...
. Modify link:{repoURL}/src/test/java/seedu/meeting/logic/parser/AddressBookParserTest.java[`AddressBookParserTest`] to test that the correct command is generated according to the user input.
...
|`* *` |new user |quick rundown of functionalities |fully understand the software features
