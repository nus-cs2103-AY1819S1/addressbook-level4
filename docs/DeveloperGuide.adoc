...
The ToDoList mechanism is facilatated by 'ToDoList', which is a differrent but similar class form 'Schedule'.
Inside the 'ToDoList', there are UnDuplicated 'ToDoListEvent' which include 'Title', 'Description' and 'Priority'.
User can add events to ToDoList with 'Title', 'Description' and 'Priority'.
The toDoList will display events in priority-decreassing order.
Also, if click the completed event, it will be deleted from toDoList.

Model for toDoList feature:
* `Priority` -- Represents the priority of a todoList event.
* `ToDoListEvent` -- The structure of single event in toDoList.
* `TodoList` -- Wraps all data at the toDoList level. uplicates are not allowed.
* `UniqueToDoEventList` -- A list of todoEvents that enforces uniqueness between its elements and does not allow nulls.
* `ReadOnlyToDoList` -- Unmodifiable view of a ToDoList.
...
Step 6: Following that, a new `CalendarEvent` is initialised and will be displayed in the list of calendar events in the GUI.
...
The calendar events are currently only fully visible via the console. Further enhancement would be needed to display the events in an appropriate Calendar GUI.
