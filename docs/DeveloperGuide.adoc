...
Each `Appointment` object consists of a `Date`, `Time`, `Patient` and `Optional<Staff>`. An `Appointment` object must have 1 `Date`, and `Time`. `Patient` contains an `Optional<Staff>#preferredDoctor` and is assigned to the `Appointment` object. `UniqueAppointmentList` then contains 0 or more `Appointment` s.
...
In future implementations, i.e. v2.0, the `Patient` object will be checked if it exists within the `UniquePatientList` before constructing the `Appointment` object. This ensures that the `Patient` is registered before making an appointment.

==== `addappt` Command

The `addappt` command behaves similarly to the `add` command used for `Patient` and `Doctor`. The command takes in the parameters required to construct `Date`, `Time` and takes in `Patient` details. Patient parameters will be simplified upon the complete implementation of the `Patient` class  in v2.0. +

The image below illustrates the process from the input `addappt` all the way to the addition of the new `Appointment` object into the `UniqueAppointmentList`.
