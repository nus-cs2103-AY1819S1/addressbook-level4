...
Tags and categories are single-word keywords tied to individual entries. Each `ResumeEntry` can classified under one `Category`, but can be associated with multiple `Tag`.
...
If there comes a new to set a new source list for the `CategoryManager`, create a new instance with the corresponding `List<ResumeEntry>`.
