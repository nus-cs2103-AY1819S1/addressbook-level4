...
=== Leave Application
In OASIS, an employee can make a leave application by specifying a description, as well as the dates, that he or she wishes to apply for.
...
===== Applying for leave
The following sequence diagrams illustrate how an application for leave by an employee works:
...
[NOTE]
There are further interactions of the EventCenter reacting to the event raised, but they are ommitted.

Zooming in to the Logic Component:
...
[NOTE]
There are further interactions within the Model component that are omitted in this diagram.
...
1. The user executes the `leave add -de family holiday -da 2018-11-11` command. The `LeaveApplication` will be initialized with the specified `Description` (family holiday), and one or more `Date` (2018-11-11), and its `LeaveStatus` will be the initial value of `PENDING`.
...
3. In the `Storage`, the `LeaveApplication` will be copied and transformed to become an `XmlAdaptedLeaveApplication` object, which is then added into the `XmlAdaptedPerson` representing the person who applied for the leave, and finally saved into a file by the <<25-storage-component, Storage>> component.

===== Viewing leave applications
Employees can view their leave applications using the `leavelist` command. The following activity diagram summarizes how the command is executed, depending on what permissions the user has:

.How the leave application list displayed is filtered currently
image::LeaveViewActivityDiagram.png[width="600"]
