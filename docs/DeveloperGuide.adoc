...
For example, `Logic`(see the class diagram given below) defines it's API in `Logic.java` interface
...
.  `Logic` uses `ShedulePlannerParser` to parse user command.
...
* stores a `UserPref` object that represents user's preferences.
...
Step 3. The user executes `add n/David ...` to add a new task. The `add` command calls `Model#commitSchedulePlanner()
...
[NOTE]
If the `currentStatePointer` is at index 0(i.e the initial schedule planner state), then there are no previous
schedule planner states to restore to. The `undo` command uses `Model#canUndoSchedulePlanner()` to check this case.
...
[NOTE]
If the `currentStatePointer` is at index `schedulePlannerStateList.size() - 1` (i.e pointing to the latest schedule
planner state), then there are no latest schedule planner states to restore. The `redo` command uses `Model#canRedoSchedulePlanner()` to check this case. If so, it will return an error to the user rather than attempting to perform the redo.
...
pointing to the latest state in the `schedulePlannerStateList`, all schedule planner states after the
...
* **Alternative 1 (current choice):** Saves the entire schedule planner
...
We are using `java.util.logging` package for logging. `LogsCenter` class is used to manage logging levels and1
destinations.
