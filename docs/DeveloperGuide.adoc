...
In order to enable users to better monitor their tasks in the schedule planner, Two more variations of
...
`listday`/`listweek` mechanism utilises the same implementation used by
`list` command:
...
based on the result of `numDaysTillSunday(dateName)` and insert into `dateList` . `dateList` is a `List<String>` object.
...
image::ArchiveDiagram.png[width="800"]
...
image::AutoDeleteArchived.png[width="800"]

Given below is detailed explanation of how auto-deletion mechanism behaves: +

. Application is launched
. `init()` method in `MainApp` class is called, which then instantiates `Model` class and calls `autoDeleteArchived()`
method in `Model` class.
. `autoDeleteArchived()` method in 
