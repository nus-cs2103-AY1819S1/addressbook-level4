...
image::actdiagram-find.png[width="880", align="left"]
...
Step 1. The user executes the `select` command on the 10th car park in the list.

Step 2. The user executes the `notify` command and indicates the interval to be 10 seconds. The `NotifyCommandParser#parse()`
gets called This means that every 10
seconds, the user will receive a notification on the number of available lots left for the 10th car park.
