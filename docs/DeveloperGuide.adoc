...
Hallper is a desktop application aimed at serving JCRCs of the Halls of NUS.
Hallper follows an event-driven architecture, and is designed to work on Mac, Linux, and Windows.
...
. The user executes the `Compose` command, which creates an email.
The `Compose` command calls `Model#saveEmail(Email email)`, saving the email to `EmailModel`.

. Once the email is saved in the `EmailModel`, the `ModelManager` raises an `EmailSavedEvent`, to indicate that a new email is saved to the `EmailModel`.

. The `EmailSavedEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleEmailSavedEvent(EmailSavedEvent event)`, which then calls `EmailDirStorage#saveEmail(EmailModel email)`.
This saves the email to a specified local directory.

. A preview of the email is displayed on the main window.

==== Design Considerations

===== Aspect: File format

The `.eml` file format is compliant with the RFC-822 format, which is industry recognised, and can be opened and viewed using many applications.
`.eml` files can also contain attachments.
...
The `.ics` file format is compliant with the RFC 5545 format, which is industry recognised, and can be opened and viewed using many applications, including Microsoft Outlook, Google Calendar, and Apple Calendar.
