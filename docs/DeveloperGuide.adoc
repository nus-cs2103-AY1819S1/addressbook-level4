...
** Cons: There is a slight delay before the drop down list appear. Can potentially slow down user's who type fast.
...
[NOTE]
As of v1.4, `PresetPermission` only contains the following: `Admin`, `Manager` and `Employee`.
...
Before any command executes, the command will first check if the logged in user possess the correct authorization by comparing `requiredPermission` with the user's `permissionSet` object. 
...
** Cons: User will have to remember 2 commands. In addition to this, User also have to execute at least 2 command if they wish to both add and remove permission. 
...
When `ViewPermissionCommand` is executed, it will retrieve the targeted employee from the `Model`, and print out the list of permission that the employee possess.
...
* Alternative 1 (Current Implementation): Display permissions as the result of a command.
