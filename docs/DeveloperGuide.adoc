...
The _User Interface_ (UI) consists of a `MainWindow` that is made up of different components.
Please refer to _Figure 5_ below for more details on how they are connected.

.Structure of the UI Component
image::UiClassDiagram.png[width="800"]

All of these components, including the `MainWindow`, inherit from the abstract `UiPart` class.

The link:{repoURL}/src/main/java/seedu/address/ui/Ui.java[`Ui.java`] uses _JavaFx UI_ framework. The layout of these _UI_ components are defined in matching `.fxml` files
