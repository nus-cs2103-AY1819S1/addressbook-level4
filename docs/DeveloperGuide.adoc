...
A user is able to save their data only after they have logged in/registered an account.

[NOTE]
Data refers to user's attribute listed in `SaveCommand` under current implementation of the save user feature.

This section will describe in detail the current implementation and the design considerations of the save user feature.

==== Current Implementation
The save mechanism is facilitated by the following classes:

* `SaveCommand` +
It stores the following attributes of an `User` object: +
...
* `UserStorage` +
It provide methods to save the current user's attributes as well as to load a file containing previously saved data. +
Additionally, it implements the following operations(s):
...
[NOTE]
Both admin and student are able to use the save feature.
...
Parsing of command is performed by `SaveCommandParser`, which returns a `RegisterCommand` object after parsing the save file path. The sequence diagram shown below in Figure 2 illustrates the interactions between the Logic & Model components when the SaveCommand is being executed.
...
The `SaveUserChangedEvent` mentioned above is handled by the `Storage` component as show in Figure 4.
...
Both the conversion of `User` object to `XmlSerializableUser` as will as writing to file is shown in the following code snippet below:
...
----
The converted newly created `XmlSerializableUser` object is saved to the file using `XmlUtil#saveDataToFile(...)` as shown in the code snipplets below:
...
1. The user launches the application for the first time and since the `currentUser` is not instantiated, the user is unable to use the save feature.

2. Having already registered an account, the user can proceed to log into their account and add their preferred modules.

3. Upon issuing the command `save sp/userdata.xml`, the save command will call Model#saveUserFile(...) which then raises `SaveUserChangedEvent`. This `SaveUserChangedEvent` will be handled by `StorageManager`.

4. `StorageManager` then utilizes `XmlUserStorage#saveUser(...)` which does the saving of the file.


[NOTE]
A new registered `Student` would be initialized and automatically set as the `currentUser`, enabling the user to perform save command. +
