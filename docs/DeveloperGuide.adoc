...
(For all use cases below, the *System* is `Picconso` and the *Actor* is the `user`, unless specified otherwise)
...
3.  User is shown a preview of the outcome
4.  User is can save the outcome/transformation to disk
+
Use case ends.
...
+
[none]
** 1a1. Picconso shows an error message.
+
...
2.  Picconso displays transformations done on the right side pane
3.  User enters command to save the set of transformations
4.  Picconso requests for a name for the set
5.  User enters a name
6.  Picconso saves the set
+
Use case ends.
...
+
[none]
** 5a1. Picconso asks if user wants to overwrite to previously stored set.
+
...
1.  User opens an image
2.  Picconso displays the image in the preview pane
3.  User inputs caption or watermark text into input
4.  Picconso displays the caption/watermark in the preview pane
+
Use case ends.
...
.  Export images in differnt formats and sizes.
