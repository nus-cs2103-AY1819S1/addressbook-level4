...
Effective Due Date is a derived field in the UI. It is the earliest due date among all tasks directly or indirectly
dependent on the task, including the task itself.

image::EffectiveDueDateImplementation.png[width="800"]
...
2. Invert the dependencies in the `DependencyGraph` +
2. Perform a depth first search from Task A
