...
AttributePredicate to test the attriute
...
A new instance of `SuggestCommand` is created whenever `AddressBookParser` fails to find a matching CommandWord,
or when {tab} is pressed inside the CommandBox.

When an incomplete command is entered, the `AddressBookParser` will instantiate a `SuggestCommand`.
The `AddressBookParser` then checks that the SuggestCommand is valid, if not throw `ParseException`.
The application will then proceed with executing the `SuggestCommand`, which will return a list of matching commands.
This list is reflected in both `CommandResult` and

Given below are some example usage scenarios and how the undo/redo mechanism behaves at each step.

image::AutocompleteSuggestActivityDiagram.png[width="650"]
Figure 3.5.1 Activity Diagram of user executing command

==== Design Considerations

===== Aspect: Handle tab through events and SuggestCommand

* **Alternative 1 (current choice):** Handle tab through events and SuggestCommand.
** Pros: Flexible.
** Cons: Difficult to implement.
* **Alternative 2:** Handle everything in CommandWindow.
** Pros: Easy to implement.
