...
[NOTE - MUST CHANGE]
As a more OOP model, we can store a `Tag` list in `LearnVocabulary`, which `Word` can reference. This would allow `LearnVocabulary` to only require one `Tag` object per unique `Tag`, instead of each `Word` needing their own `Tag` object. An example of how such a model may look like is given below. +
 +
