...
image::AddmhSequenceDiagram.png[width="800"]

* `addmh ic/S1234567A mh/Hypertension, diagnosed “years ago”, well contracted with Metoponol doc/Dr. Amos`

** Firstly, the user input is passed into the `LogicManager::execute` method of the LogicManager instance as the only parameter.

** Then, within the `LogicManager::execute` method the AddressBookParser::parseCommand instance method receives the user input as a parameter.
*** This `String` argument is formatted: the first `String` token is taken as the command word, while the remaining `String` is grouped as arguments to be used later in `AddmhCommandParser`.
...
*** The `AddressBookParser ` then calls the `AddmhCommandParser::parse` method.

**  Next, the `AddmhCommandParser` takes in the previously mentioned remaining arguments string, `ic/S1234567A mh/Hypertension, diagnosed “years ago”, well contracted with Metoponol doc/Dr. Amos`
*** From this string, the relevant arguments are tokenised based on the set of prefixes involved.
...
*** If not all prefixes are present, a `ParseException` will be thrown with a error message on the proper usage of the `addmh` command.
*** Otherwise, `Diagnosis` and `Nric`  objects are constructed and used as fields in the creation of an `AddmhCommand`.

** Subsequently, the newly created `AddmhCommand` is returned to back to the `LogicManager` instance through `AddmhCommandParser` and `AddressBookParser` objects.

**  When control is returned to the `LogicManager` object, it calls the `AddmhCommand::execute` method.
...
public CommandResult execute(Model model, CommandHistory history) throws CommandException {
    ...
    Person patientToUpdate = getPatient(model); // <1>
    Person updatedPatient = addMedicalHistoryForPatient(patientToUpdate, this.newRecord); <2>

    model.updatePerson(patientToUpdate, updatedPatient); <3>

    return new CommandResult(String.format(MESSAGE_SUCCESS, patientNric)); <4>
}
----
...
* The stored persons data is accessed in the AddmhCommand::getPatient method.
* Model::getFilteredPersonList is called to search for a person with a `Nric` that matches the `Nric` field in the `AddmhCommand`
* If a match is found, the `Person` is returned to the `AddmhCommand::execute` sequence.

<2> addMedicalHistoryForPatient() method:
+
[source, java]
----
private static Person addMedicalHistoryForPatient(Person patient, Diagnosis diagnosis) {
    requireAllNonNull(patient, diagnosis);

    MedicalHistory updatedMedicalHistory = patient.getMedicalHistory();
...
----
* Updates the person’s medical history.
...
----
* Replaces the old person data with the updated person’s data.
* Here the Model::updatePerson method is called, and it subsequently calls the VersionedAddressBook::updatePerson method
* Replaces the person’s existing data in the addressbook storage with the person’s updated data.

<4> The `AddmhCommand::execute` execution completes by returning a new `CommandResult` that contains a success message to its calling method, `LogicManager::execute`.

* Command execution ends.

---
The activity diagram in Figure 13 summarises what happens when a user executes the `addmh` command.
...
image::VisitorinSequenceDiagram.png[width="1000"]
image::ViewvisitorsSequenceDiagram.png[width="1000"]
image::VisitoroutSequenceDiagram.png[width="1000"]
