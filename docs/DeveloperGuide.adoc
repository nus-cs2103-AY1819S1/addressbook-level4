...
* Aspect: How `readLibrary` executes to access mp3 files in library folder.
** Alternate 1: (Current choice): Read the Json file from data folder and scan the tracks from library folder at the same time.
*** Pros:
...
** Alternate 2: Do not scan the tracks in `readLibrary`, scan the library each time the user search for tracks in the library folder.
*** Pros:
...
** Alternative 1 (Currently Choice): List the tracks in a new panel separate from playlist panel.
*** Pros:
