...
A call to 'BudgetPanel#incrementKeyFrames()` is made to create the incrementing animation for `BudgetPanel#expenseDisplay`.

The total time for this animation is 0.5 seconds, and consists of 10 `KeyFrame` objects.

With this information, the amount to increment `BudgetPanel#expenseDisplay` by in each `KeyFrame` can be calculated.

`BudgetPanel#updateExpenseTextDisplay()` is called to increment the `BudgetPanel#expenseDisplay`.

image::BudgetPanelTextKeyFrameDiagram.png[width="800"]
