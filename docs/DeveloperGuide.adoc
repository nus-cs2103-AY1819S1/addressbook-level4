...
**Step 2** +
When updateUpcomingEvents(eventToEdit, editedEvents) in `ModelManager` is called, this method will call editUpcoming(eventToEdit, editedEvents) in `PopUpManager`, which
runs deleteUpcoming(..) and then add(..) +

**Step 3** +
deleteUpcoming will delete all `EventPopUpInfo` objects that share the same `SetId` with eventToEdit but have
