...
Translating from `MeetingBookStorage` to XML formatted file is handled by `XmlFileStorage#saveDataToFile`.
...
Translation from XML formatted file to `XmlSerializableMeetingBook` is handled by `XmlFileStorage#loadDataToFile`.
...
Each ``Object``s has their respective `XMLAdapted` class which contain directions to model back from JAXB-friendly version . `Unmarshal` Object is solely responsible for disassemble the XML file into  JAXB-friendly versions of `Person`, `Meeting`, `Group`.
