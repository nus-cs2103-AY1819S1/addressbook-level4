...
. Modify permission of an employee while active persons are listed

.. Prerequisites: Logged in as a user with `ASSIGN_PERMISSION` permission. List all persons using the `active` command. At least one person in the list. Employee at index 1 without any permission.
