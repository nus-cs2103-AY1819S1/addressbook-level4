...
.. Test case: `notification n/warning t/on` + `add n/test c/test $/11.00`.
   Expected: A notification should appear on the the `NotificationPanel` with the header `Warning` and the
...
.. Other incorrect delete commands to try: `notification`, `notification x` where x is any input,
   `notification n/x` where x is any input other than `tip` or `warning`, `notification n/warning t/x` where
