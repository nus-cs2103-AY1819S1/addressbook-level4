...
to henceforth as _document related commands_, all inherit from it. It specifies what general information
 these documents should have and how they should be formatted. It can hence be thought of as a
 *formatting template*.

As the template on which the documents depend on, the `Document` class itself *cannot* be instantiated, hence
implemented as an `abstract class`. The `Document` class has the following key features:

* contains a `name` and `icNumber` attribute, extracted from the `ServedPatient` from which the document is constructed.
...
The following activity diagram summarises what happens internally when a _document related command_ is called.
...
image::DocumentActivityDiagram.png[width="800"]

As seen in the above activity diagram, upon executing a _document related command_, the corresponding document will be constructed from the `ServedPatient` object specified by the `index` that was passed in with the command.
Step 1. The resulting `Document` object will call the `generateFile()` method, which will first make a unique `fileName` for the file to be created, identified by the type of `Document`, `ServedPatient's` `name`, `icNumber` and current `date time`.
...
flexibility of structuring the contents unique to its document type.
** Cons:
*** Prevents `inheriting` classes from `inheriting` from other useful classes or becoming an `enum` as `Java` does not support `multiple inheritance`.
*** More computationally expensive than using an `interface` due to the overhead incurred looking up `inherited methods` and various `class members`.
...
****
`Enums` can prevent the issuing of a `Medical Certificate` that excuses a person from work/school for 10 **years**!
****
** Cons:
*** Unable to standardise the way the `inheriting` classes implement the `generateHeader` method.
****
...
* contains a `Map<Medicine, MedicineQuantity>` attribute to record the quantity dispensed of each type of `Medicine` to the `Served Patient` from which the `Receipt` was constructed from.
...
Users can generate a `Receipt` for a specific `ServedPatient` by invoking the `ReceiptCommand` accompanied by the served patient's index in the `ServedPatientList`. The below sequence diagram illustrates how the `ReceiptCommand` works.

When the `ReceiptCommand` is executed, it will construct a new `Receipt` object and extracts the relevant information from the `ServedPatient` specified by the `index`.
