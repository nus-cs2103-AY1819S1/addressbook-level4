...
By: `CS2103 T10-2`      Since: `Aug 2018`      Licence: `MIT`
...
the `AddCommand` and `AddCommandParser` such that it is able to create a `Patient` object. This command
automatically tags the object created as `Patient`.
...
the `AddCommand` and `AddCommandParser` such that it is able to create a `Doctor` object. This command automatically tags
the object created as `Doctor`.
...
Step 2. Upon hitting enter, the `Doctor` will be created and added to the storage of the system


==== Design Considerations
===== Aspect: Presenting Upcoming Appointment to Doctors
* Alternative 1 (Current Choice): Implement Google Calendar to reflect upcoming appointment to the doctor. This allow
doctors to view information provided by the receptionist without accessing HealthBook directly to check the information.

=== Remark Command
==== Current Implementation
Remark command allows user to input any information into Doctor/Patient object with no restrictions. This command can
only be called after person is created.

Step 1. The user types in `remark` in the command box followed by the parameters `index r/some remark`
...
==== Design Considerations
===== Aspect: Easier access to remark
* Alternative 1 (Current choice): Currently, the remark command is only accessible via index. We will change it to be
accessible via name as this allows the receptionist to edit the remark without looking for the index beforehand.
...
command is implemented such that it is able to add `allergies` AND/OR `conditions` to `MedicalHistory` object for a specific
`Patient` specified by index.

Step 1. The user types in `add-medical-history` in the command box followed by the parameter `INDEX al/penicillin,milk
c/sub-healthy, hyperglycemia`

Step 2. Upon hitting enter, the `allergies` AND `conditions` of `MedicalHistory` will get added

==== Design Consideration
===== Aspect: Support different types of input
* Alternative 1 (Current choice): `AddMedicalHistoryCommand` allow user to fill in one or more allergies and conditions
(separated by comma). It is also supported to leave EITHER field blank.
...
`condition` from `MedicalHistory` object for a specific `Patient` specified by index.

Step 1. The user types in `delete-medical-history` in the command box followed by the parameter `INDEX
al/penicillin,milk c/sub-healthy, hyperglycemia`

Step 2. Upon hitting enter, the `allergies` AND `conditions` of `MedicalHistory` will get deleted

==== Design Consideration
===== Aspect: Support different types of input
* Alternative 1 (Current choice): `DeleteMedicalHistoryCommand` allow user to fill in one or more allergies and
conditions (separated by comma). It is also supported to leave EITHER field blank.

=== Filter Patient and Filter Doctor Feature
===== Current Implementation
This feature help to filter all patients/doctors in health book

Step 1. The user types `filter-patient`/`filter-doctor`in the command box

Step 2. Upon hitting enter, all persons who are tagged as "Patient"/"Doctor" would appear on the panel
