...
There is a ImageMagicUtil class which will check, create, and run the imagemagick executable file,
...
*Step 2.* The login command calls `Model#getPhotoHandler()` and checks if a PhotoHandler instance already exists. If not, it calls static method `PhotoLibraryClientFactory#createClient()` to set up the requirements for log in and redirects the user to the browser.
...
*Step 3.* User logs in through redirect window and a refresh token is returned and stored. PhotoLibraryClientFactory class then calls `PhotoLibraryClientFactory#createPhotosLibraryClient()` and `PhotoLibraryClientFactory#getUserEmail()` to instantiate a `PhotoHandler` instance
...
*Step 2.* The method checks for stored credentials (refresh token), and logs in user via `PhotoLibraryClientFactory#createClient()` if possible, else the log in process is skipped.
...
=== GoogleLsCommand (ls) and GoogleRefreshCommand (refresh)
...
==== Example: First call for `g ls`, where user wants to list all albums in Google Photos.
...
*Step 3.* Within that method, it makes a request to Google Photos, retrieves a `List<Album>`, and stores them in a `Map<String, Album>` within the PhotoHandler instance, with each key being the album name, any duplicate names are renamed.
...
*Step 1.* The user executes a `g dl /i<b.png>` command.

*Step 2.* The command goes through parsing, firstly by `GoogleCommandParser` and secondly filtered by GoogleDlCommand. It is determined to be for downloading one image, and `model.getPhotoHandler().downloadImage()` is called.

*Step 3.* Within that method, it makes a request to Google Photos with the image's respective ID, and successfully downloads the image into the user's local directory.

*Step 4.* Feedback of success is sent to the user

[WARNING]
If the local directory contains any image with the same name as the one to download, the image in the local directory will be replacedm=.

.Sequence Diagram for GoogleDlCommand
image::GoogleDlSequenceDiagram.png[width="1000"]


The process is similar for the other 2 variants, except images or images from a specific album are downloaded instead.

==== Design Considerations
* to download to a specific folder in future version/to check for same-name image as well.
