...
==== Current Implementation
The medicine records system is used to manage the medicine inventory in the clinic. The _Medicine_ class and the properties
of the medicine are stored under the Model package. The list of medicines currently tracked in the clinic is located in the AddressBook class.

Click <<link, here>> for the list of commands related to the medicine class.
...
`editmedicine` to correct the serial number.
...
// sequence diagram for checkstock
...
from the `ServedPatient` specified by the `index`.
Shown below is how information like `referralContent` and `noteContent` is formatted into the referral letter document template.
``` Java
    public String formatInformation() {
