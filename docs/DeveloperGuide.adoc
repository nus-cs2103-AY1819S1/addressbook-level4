...
This allows time to be saved when trying to run a command, because instead of having to `find` a `Person` then type the command based on the `Index` of the list that the `Person` appears under, commands can be targeted swiftly and precisely.
...
Note: in the following section, the shorthand format `_ByNameCommand` and `_Command` when used in the same context will refer to a similar type of command, e.g. `DeleteByNameCommand` and `DeleteCommand`, but `__` is general to refer to either `Delete` or `Edit`.
