...
The figure below shows how the add command is processed through the logic of TheTracker:
...
the `addperson` command will be taken as the sole example in this document. The `addperson` string
input in the command line is passed into the `AddressBookParser` object, which parses the input
and creates an `AddPersonCommandParser` to help it further parse the argument and create an
`AddPersonCommand`. This `AddPersonCommandParser` creates a Person with the Compulsory attribute `Name`,
and optional fields `Phone`, `Email`, `Address` and `Tag` as provided by the user and passes this
person to be added to a new `AddPersonCommand`. The `AddPersonCommand` adds the person to the model and
commit and save the AddressBook.
