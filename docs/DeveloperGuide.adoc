...
+
This constructor is used when an event, being initialised with a new `Event` class, does not yet exist in the system (e.g. adding a new event).
+
It increments the current highest ID in the system and assigns that value to the new event that is created.

* `Event(EventId eventId, Name name, Location location, Date startDate, Date endDate,
                    Time startTime, Time endTime, Description description, Set<Tag> tags)`
+
This constructor is used when an event, being initialised with a new `Event` class, already exists in the system (e.g. loading data from XML file or editing an existing event).
+
...
* *Alternative 1 (current choice):* Increment from highest ID
+
[cols="1,10"]
|===
|*Pros*| Implementation is easy.
|*Cons*| Maintained highest ID may be susceptible to overwrite and become desynchronised.
|===
+
* *Alternative 2:* Randomly generated unique ID
+
[cols="1,10"]
|===
|*Pros*| Not dependent on a maintained highest ID variable (single point of failure).
...
[width="59%",cols="22%",options="header",]
|=======================================================================
|Use Cases
