...
When an incomplete command is entered, the `AddressBookParser` will parse the output using a `SuggestCommandParser`.
The `AddressBookParser` then checks that the SuggestCommand is valid.
