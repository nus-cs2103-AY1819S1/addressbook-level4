...
*	`BudgetPanel#budgetDisplay` – A `Text` element that displays the user’s monthly totalBudget cap.
...
. The user executes the command `add n/Lunch $/30.00 c/Food`. As the `'add` command modifies the totalBudget and expenses, `AddCommand#execute()` will post a `UpdateBudgetPanelEvent` event to the EventsCenter.
[NOTE]
...
. `BudgetPanel#update()` also calls `BudgetPanel#setBudgetUiColors()``. The method detects that the user has gone over totalBudget, and changes the color of `BudgetPanel#expenseDisplay` and `BudgetPanel#budgetBar` from green to red.
[NOTE]
Similarly, if the user has gone from over totalBudget to under totalBudget, the color of 'BudgetPanel#expenseDisplay` and `BudgetPanel#budgetBar` should change from red to green.
...
=== Budget
==== Current Implementation
The totalBudget feature is one whereby after a user sets a totalBudget, if the total spendings of a user exceeds his totalBudget
during a period determined by the user, a warning will show up to inform the user that adding an expense results in
him exceeding his set totalBudget.

The current implementation for totalBudget tracking is described as below.

===== Setting a totalBudget
Below would be how a totalBudget is set by the user.

.Sequence diagram of a user setting a totalBudget.
...
. `SetBudgetCommandParser#parse()` will then return a `totalBudget` of `double` type. It will then create a
`SetBudgetCommand` Object with `totalBudget` as a parameter would be created and returned to `LogicManager`.
. `LogicManager then calls `SetBudgetCommand#execute()`, which calls `ModelManager#modifyMaximumBudget` to update the
 maximum totalBudget of the expense tracker.
. `LogicManager will then call `EventsCenter#post()` to update the UI, displaying the updated totalBudget.
. The totalBudget is then updated.


===== Recurring totalBudget

Every time a user logs in, `Model#loadUserData()` is called and  the application will check if the user has set a time
where the
totalBudget will reset (recurrence time). If it is not set, the expense tracker will issue a notification. Else, the
expense tracker will track the set recurrence time against the time now. If it exceeds, totalBudget will be reset. Else,
...
//TODO: ADD DIAGRAM TO SHOW THIS

Below is an example of how a user sets the recurrence time.
...
. `ExpenseTrackerParser` will then creates a `SetRecurringBudgetCommandParser` Object and calls `SetRecurringBudgetCommandParser#parse()` method.
...
method to update the time where the next totalBudget is reset.
...
usage of the expense tracker, but rather provide helpful information. For example, if a user exceeds his totalBudget after
...
Users can now allocate their budgets by categories. This means that within a fixed totalBudget, they can decide how big of
 a proportion a certain category of expense takes. Every `Category` will now have a `Budget` and the total totalBudget in
...
|`* * *` |clumsy user |delete inaccurately added expenditure |have an accurate recording of my expenditures and totalBudget

|`* * *` |clumsy user |edit inaccurately added expenditure |have an accurate recording of my expenditures and totalBudget
...
|`* * *` |forgetful user |receive a notification about my totalBudget when the application boots up |be reminded of how much I have spent and can spend.
...
|`* * *` |impulsive spender |set my maximum totalBudget over a certain period of time |know when I am or about to spend over my totalBudget

|`* * *` |user who has a limited totalBudget |view my expenses over a certain period of time |learn from my past endeavours and better manage my totalBudget
...
|`* *` |user who has a monthly allowance |set my totalBudget based on my monthly allowance |
