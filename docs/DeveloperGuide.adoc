...
.Activity Diagram for ListAppt.
image::ListApptActivityDiagram.png[width=""]
...
[NOTE]
What is seen in the UI is actually the unmodifiable `ClinicIO#ObservableAppointmentList`.
Commands that require the `listappt` are actually modifying the
`Model#filteredAppointmentList` and then updating the `ClinicIO#ObservableAppointmentList`. (
...
has two parts, `cancelAppointment` and `removeAppointment`. `cancelAppointment` simply changes the status of the appointment and
adds it to the `Patient` s `AppointmentHistory` (not shown). `removeAppointment` actually removes the appointment from the
