...
* `addmh ic/S9123124A mh/Hypertension, diagnosed “years ago”, well contracted with Metoponol`
** When the user-executed `addmh` line is passed into an instance of the LogicManager class, the method `HealthBaseParser::parse` is executed.
`HealthBaseParser` then parses the command word `addmh` and subsequently executes `AddmhCommandParser::parse`.
