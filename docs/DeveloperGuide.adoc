...
1) Not all commands can undo or redo. Some commands, such as `list`, do not modify the address book and
...
2) Some commands such as `clear`, will calls `Model#commitAddressBook()`. In this case, the `currentStatePointer` is
...
* `EmailModel#saveEmail(Email email)` -- Stores each newly composed email in the `EmailModel`.
* `EmailDirStorage#saveEmail(EmailModel email)` -- Saves the newly composed email in the `EmailModel` to the computer.

These operations are exposed in the `Model` interface as `Model#saveEmail(Email email)`, and in the `Storage` interface as `Storage#saveEmail(EmailModel email)` respectively.

Given below is an example usage scenario and how the Compose feature behaves at each step:

1) The user executes the `Compose` command, which creates an email.
The `Compose` command calls `Model#saveEmail(Email email)`, saving the email to `EmailModel`.

2) Once the email is saved in the `EmailModel`, the `ModelManager` raises an `EmailSavedEvent`, to indicate that a new
 email is saved to the `EmailModel`.

3) The `EmailSavedEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleEmailSavedEvent
...
4) A preview of the email is displayed on the main window.

==== Design Considerations

===== Aspect: File format

* **Pros:** The `.eml` file format is compliant with the RFC-822 format, which is industry recognised, and can be
opened and viewed using many applications.`.eml` files can also contain attachments.
* **Cons:** `.eml` file alone is not useful alone. More implementations are required to make `.eml` useful in Hallper.

===== Aspect: Using http://www.simplejavamail.org/#/about[Simple Java Mail]

http://www.simplejavamail.org/#/about[Simple Java Mail] contains various methods to conveniently create emails.
It can also be used to send emails if Hallper is extended to include online capabilities.
The library is easy to understand so any new developer can easily extend the current features.

===== Aspect: HTML Text

User input for email content is saved as HTML Text so that users with HTML Text Formatting knowledge can easily format the body of the email.
//end::compose[]
...
It allows `.ics` files to be saved onto the computer. Calendars in Hallper are created as monthly calendars, a `Map<Year, Set<Month>>` in `CalendarModel` keeps a record of existing calendars in Hallper.
...
The `create_calendar` command then calls `Model#isExistingCalendar(Year year, Month month)`, to check whether the calendar already exists inside Hallper. If it exists, the command does nothing and reflects to the user that the calendar already exists. Else the command will then call `Model#createCalendar(Year year, Month month)`, initializing a calendar object inside `CalendarModel`.

. Once the calendar object is initialized in the `CalendarModel`, the `ModelManager` raises a `CalendarCreatedEvent`, to indicate that a calendar object has been initialized in the `CalendarModel`.

. The `CalendarCreatedEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleCalendarCreatedEvent(CalendarCreatedEvent event)`, which then calls `IcsCalendarStorage#createCalendar(Calendar calendar, String calendarName)`.
...
The `.ics` file format is compliant with the RFC 5545 format, which is industry recognised, and can be opened and
viewed using many applications, including Microsoft Outlook, Google Calendar, and Apple Calendar.
...
[none]
* 1a. JCRC member requests to clear persons with specific tag
+
[none]
** 1a1. Hallper clears all persons with specified tag
+
Use case ends.
...
1. JCRC member requests to import contacts
2. Hallper requests for file location
3. JCRC member enters file location
4. Hallper imports contacts
+
Use case ends.
...
1. JCRC member requests to erase a specific tag
2. Hallper erases specified tag from all persons
+
Use case ends.

[discrete]
=== Use case: Compose email

*MSS*

1. JCRC member requests to compose email
2. Hallper requests for all details of email (Sender, recipient, subject, content, attachment)
