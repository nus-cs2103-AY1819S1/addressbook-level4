...
* **Alternative 1 (Current choice):** Extracting the location from the previous `FindCommand` to form part of the predicate for the current `FilterCommand`
+
[cols="1,10"]
|===
|Pros| Able to creating a combined predicate that does location-based filtering easily.
Various existing methods that filters the list of car parks can still be used.
|Cons| Efficiency compromised as the list of car park needs to be filtered by location again.
|===
...
//** *Pros:* Easy to implement
//** *Cons:* Have to decide on each abbreviation subjectively which brings down performance if there are a lot of commands
