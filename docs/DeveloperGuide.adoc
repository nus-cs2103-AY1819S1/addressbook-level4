...
. Fix `invalidAndValidPersonAddressBook.xml`, `typicalpatientsAddressBook.xml`, `validAddressBook.xml` etc., such that the XML tests will not fail due to a missing `<remark>` element.
...
. Try and use the method on any sample `Person` in link:{repoURL}/src/test/java/seedu/address/testutil/Typicalpatients.java[`Typicalpatients`].
...
|`* * *` |user |find a person by name |locate details of patients without having to go through the entire list

|`* *` |user |hide <<private-contact-detail,private contact details>> by default |minimize chance of someone else seeing them by accident

|`*` |user with many patients in the address book |sort patients by name |locate a person easily
|=======================================================================
...
1.  User searches the name of a person
2.  HMS shows a list of patients
...
. Deleting a person while all patients are listed
