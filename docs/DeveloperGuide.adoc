...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `GenericListPanel`, `StatusBarFooter`, `DetailPanel` etc. All these, including the `MainWindow`, inherit from the abstract `UiPart` class.
...
.  `AppContentParser` passes necessary model to each feature parser such as `IngredientParser` according to the context.
.  This results in a `Command` object which is executed by the `LogicManager`.
.  The command execution can affect the `Model` (e.g. adding an ingredient) and/or raise events.
...
.Interactions Inside the Logic Component for the adding an ingredient
...
Ingredient Manager is an address book of ingredients. With the basic functions from a normal address book such as
add, edit, delete and list, Ingredient Manager has some useful functions specific for management of ingredients. +
First of all, serving units can be converted to common units with reference to serving unit dictionary.
Conversion to common units is necessary to enable arithmetic calculation for amounts. Also, it is easier to compare
between ingredients when the amounts are shown with common units. Secondly, when you apply the list command,
ingredients are sorted by date. Since ingredients have to be consumed in a certain period, it keeps user be reminded of old ingredients.

===== Current implementation for serving units dictionary
Ingredient Manager uses gram for common units. Serving unit dictionary is implemented with `static HashMap` in
`IngredientServingUnit` class, which
contains
 names of a serving units as a key and IngredientServingUnitDefinition objects as a value.
 `IngredientServingUnitDefinition` has three fields, which are a serving unit to be converted, a common serving unit,
  and
 a constant to be multiplied for conversion. When conversion is to be done, you find the value in the HashMap with a
 input serving unit as a key. Then, you multiply the amount of ingredient with the constant and replace the unit with
  common unit. This process is wrapped in a method named `convertToCommonUnit` in ingredient models containing amount
  as a field.

===== Alternative Implementation for serving units dictionary
In the current implementation, a serving unit to be converted is used both as a key of the HashMap and one of the
fields
in value of the HashMap. This duplication can be removed by replacing the value of the HashMap with array
containing other two fields. However, wrapping the three fields in a separate class is desired for abstraction.
Although in our current application only make use of the class for conversion to common unit, the direction of
conversion can be also done the other way around for display purpose. Also, in a more complete version of the
dictionary, the definitions needed for conversion should also contain information related to a specific ingredient so
 that the dictionary considers the validity of the combination between an ingredient and serving units. The
 duplication between key and value in HashMap can be cancelled out with the convenience in implementation obtained
 from choosing HashMap that other data structures for searching in dictionary.
...
image::IngredientModelDiagram.png[width="1000"]

Currently, there are three ingredient related models, namely `IngredientDefinition`, `IngredientPortion`, and
`Ingredient` where IngredientPortion inherits IngredientDefinition, and Ingredient inherits IngredientPortion.
IngredientDefinition is a wrapper class for ingredient name. Ingredient portion additionally includes amount as
field, and Ingredient is the most complete version of ingredient used by ingredient manager with date information.
The reason for this hierarchical implementation is that there exists different needs for ingredient related fields
across features and functions. For example, ingredient in recipe uses IngredientPortion, and ingredient in Ingredient
 Manager uses Ingredient. IngredientDefinition is used as a key of a HashMap when implementing the "view recipes
 according to ingredients" function. It can be potentially used to construct ingredient dictionary and more complete
 serving unit dictionary in the future version of the application.
...
While browsing the favourite recipes in the favourite context, and when a user likes a recipe at a particular `INDEX`, they can
execute the `favourite INDEX` command. The recipe selected would be copied from the recipe model, and added to an array
list in the favourites model.
...
The command replaces the existing UniqueList of _favouriteModel_
with a new empty UniqueList.
...
`IngredientPortion` from each `Instruction` of every recipe.
...
image::recipe_class_diagram.png[width="1000"]

Other than the basic `Recipe` details accessible via the `getters()`, nested attribute such as `IngredientPortion`
can post difficulty for the search function since `Recipe` and `IngredientPortion` is not associated initially. To
allow direct access to all `IngredientPortion` of that recipe, a `tabulateIngredients()` method is added. This method
stores all
`IngredientPortion` in the `HashMap` of `ingredients`
 attribute by searching through all `Instruction`. As `Recipe` is immutable, the generating of the `ingredients` is
 done in the constructor of `Recipe` (i.e. `tabulateIngredients()` is called in the constructor). This allow the
...
include::../src/main/java/seedu/souschef/model/recipe/RecipeContainsKeywordsPredicate.java[lines=19..33]
...
** Pros - Remove the need to search through the initial every instruction to gather the ingredients for every new
instance of recipe. Speed up the recipe add and read process. +
** Cons - Since ingredients are aggregated from instruction, by allowing IngredientPortion to be a direct attribute of
recipe imply a possibility of mismatch between the ingredients in the instruction and the ingredients in the recipe.
(e.g. User entering ingredients that are not used by any instruction)
...
*Value proposition*: Plan and guide health-conscious occupied working adults in preparing home-cooked meals
...
[discrete]
==== Use case: UC44 – Navigate Recipe [Coming up in V2.0]
*MSS*
