...
//TODO Explain XML
The program will raise a `AddressBookExportEvent` to `EventCenter`. `StorageManager` which subscribed to the `EventCenter` which will trigger `handleAddressBookExportEvent` when received `AddressBookExportEvent`. This method will then export the data as a XML file using `Model#exportAddressBook`.

===== Exporting to XML
As MeetingBook stores `Person`, `Group`, `Meeting` as ``Object``s, these information have to be properly formatted into XML-like syntax before it can be successfully saved.
``Object``s are parsed using their respective `XMLAdapted` classes which contain directions for it to parse into XML-like syntax.
