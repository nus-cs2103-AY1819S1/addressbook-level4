...
** Additioanl details: A `String personIdentifier` will be stored in the command, and upon `execute`, a person is first matched, then the edit is carried out.
...
image::PictureCommandActivityDiagram.png[width="800"]
...
** Cons: More memory is used as there is a need to store an object. A new `Picture` class have to be made and implemented.
