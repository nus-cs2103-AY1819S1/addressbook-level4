...
The model contains different methods to update the various lists. A predicate is passed to help to reset each of the lists to the initial state.
...
From the code snippet above, we see that the the current state of event list from the model is stored into another another list called 'filteredEventList'.
This is so that the list can be easily referenced in later parts of the code.
...
If the user input is valid, the application can change to the record context. The selected event by the user is also stored in the model.
The model also resets the state pointer so that the undo and redo functions will point to a fresh, new state.
