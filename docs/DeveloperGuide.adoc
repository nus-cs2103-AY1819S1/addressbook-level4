...
components of EventOrganiser, namely, the EditUser Command, and the DeleteUser Command,
...
his/her account. However, due to the way users are uniquely identified in EventOrganiser,
...
the Model can call the correct Event 'e' and Person object 'user' in the `addVoteToPoll` method. If there is no logged-in user,
...
This method returns an empty list if no user has voted for any option yet.
