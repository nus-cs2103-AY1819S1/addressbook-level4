...
Classes used by multiple components are in the `seedu.expensetracker.commons` package.
...
* `EncryptionUtil#encryptTracker(ExpenseTracker)` -- Creates an EncryptedExpenseTracker instance of the given ExpenseTracker by encrypting it using its encryption key. This is always called when saving users' data to storage.
...
* The encrypted information are stored in new classes to ensure that encrypted data is not used before decryption.
* The encrypted information has to be stored in the `Model` as the encryption key will only be known at runtime when a user logs in with his/her correct `Password`.
...
image::implementation/StatsPreparationSequenceDiagram.png[width="800"]
...
image::implementation/StatsExecutionSequenceDiagram.png[width="800"]
...
. `login` command executes in the `Logic`, which calls `Logic#execute()` to execute the command on the `Model`
