...
* can save the Health Book data in xml format and read it back.
...
does not login to google, subsequent access to their google calendar will be denied. However, doctor object will still
be created.
...
* Alternative 2 (Alternative Choice): Doctor object will not be created if Google Login is unsuccessful, instead throw
an exception. This will make Google Calendar mandatory for all doctors.
...
.Sequence Diagram for Remark Command
...
[NOTE]
For multiple inputs in the same field, use comma `,` to separate. Do not append blank after `,`.
You must provide both prefixes of allergy and condition, but can leave at most one input blank.
