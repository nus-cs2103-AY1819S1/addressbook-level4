...
CLInic is a desktop application that is created for usage within a clinic, and optimised specifically for clinic receptionists and doctors who *prefer to work with a Command Line Interface* (CLI) while still having the benefits of a Graphical User Interface (GUI).

If this is your first time on this application, proceed to <<Setting up>> to get started with setting up your development environment. Following which, you can proceed to *Appendix A* to get start on your first task!
...
The `MedicalRecord` class is an attribute of the `Patient` class, and it contains information regarding its `Patient` 's medical records. The `MedicalRecord` class structure is as follows:
...
image::addmedicalrecord-addingnewpatient-example.png[width="700"]
...
.Adding a new MedicalRecord with only BloodType specified.
image::addmedicalrecord-addingbloodtype-example.png[width="700"]
...
* **Alternative 1 (current choice):** Create a new `MedicalRecord` object and combine the two.
...
// tag::medicine[]
=== Medicine records
The medicine records is used to manage all medicines stored in the clinic. The AddMedicine Command helps users to
key in a new medicine in the clinic. EditMedicine Command helps users to update the stock. In addition, whenever the
Receipt Command is called, the records will update the stock automatically when the receptionist generates the receipt and
issues the medicine.

Given below is an example of how the medicine class is used:

Step 1. A new medicine arrived. The receptionist uses the `addMedicine` command to add the stock into the records.

Step 2. A new stock of medicine that ia available in the clinic has arrived. The receptionist uses `editMedicine` command
to update the new stock.

Step 3. When a patient pays for the bill, the `generateReceipt` calls upon the medicine records to determine the price.
At this stage, the number of medicine issued will decrease in the records.

Step 4. At the end of the day, the receptionist needs to keep track of which stock supply is low. She enters 'checkStock`
which will list out all medicien that are low in supply.
//end::medicine[]

// tag::pqms[]
=== Patient Queue Management System
The Patient Queue Management System consists of three main classes: `PatientQueue`, `CurrentPatient` and `ServedPatientQueue`. A patient that
registers to see the doctor is put into the `PatientQueue`, when it is his turn to see the doctor he will be the `CurrentPatient`. After
he is done with seeing the doctor he will be put into the `ServedPatientQueue` to await payment and document processing.

Given below is an example usage scenario and how the Patient Queue Management System is used in each step.
...
Step 3. The user executes various commands to add information to the `CurrentPatient`. These information will be used for document
processing later on.

Step 4. The `CurrentPatient` is done with the consultation. The user executes the `finish` command to transfer this patient to
the `ServedPatientQueue`. Afterwards, the user execute `serve` to get the next patient.
...
Step 6. The user finally executes `payment` command to generate the receipt and end the patient's visit to the clinic, removing
him from the `ServedPatientQueue` and adding him into the `ServedPatientList`.
//end::pqms[]

// tag::document[]
=== Document
==== Current Implementation
The `Document` `abstract class` covers all the documents that are issued by the clinic. Hence, the `Receipt`, `MedicalCertificate` and `ReferralLetter` classes all inherit from it. +
It specifies what general information these documents should have and how they should be formatted. It can be thought of as a template for all its inheriting classes.
...
* contains all the `Medicine` and their associated `MedicineQuantity`.
** These are the medicines and their quantities respectively, prescribed by the doctor to the patient during his consultation.

Users can generate a `Receipt` for a specific `ServedPatient` by invoking the `ReceiptCommand` accompanied by the served patient's index in the `ServedPatientList`. +

Given below is an example usage scenario and explanation for the inner workings of the `ReceiptCommand`.
...
Step 2. The user executes `receipt 1` command to generate a `Receipt` with the relevant object. Up till this milestone v1.2, the contents of the `Receipt` will be reflected in the `ResultDisplay` as shown below.

.Contents of the `Receipt` generated for the `Patient` Bryan Lew. +
image::ReceiptExamplePrintOut.png[width="650"]

[WARNING]
Executing `receipt` with an invalid index will **not** result in the generation of a `Receipt`.

.Result of executing `receipt` with an invalid index.
image::ServedPatientListEmptyReceipt.png[width="650"]
...
** Pros:
*** Allows all classes inheriting from `Document` to use a standardised method to format the relevant information that appears in every document that is issued by the clinic.
** Cons:
*** `Java` does not support multiple inheritance; therefore, it prevents the classes that are inheriting from `Document` from inheriting from other useful classes or becoming an `enum`.
*** Comparatively, `abstract classes` are more computationally expensive to use than `interfaces` due to overhead incurred looking up methods and various class members.
* **Alternative 2**: Using an `interface` to implement `Document`.
** Pros:
*** The classes implementing `Document` can be converted to `enums`, which ensures that the inputs fed to these classes are valid.
**** For example, a person should not be issued a `Medical Certificate` excusing him from work/school for **1,000,000 days**!
** Cons:
*** Unable to standardise the way the implementing classes implement the `generate` method.
**** This can potentially allow the inheriting classes to omit certain fields that are relevant to the printing of a document, such as `date` and `time` of the consultation.
**** Even if all the information are present, they might be formatted differently.
//end::document[]
