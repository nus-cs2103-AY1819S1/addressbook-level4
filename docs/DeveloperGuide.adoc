...
image::AddmedsSequenceDiagram.png[width="650"]

A more detailed explanation of the command execution can be found below.

The medication storage and retrieval is facilitated by the following classes:
...
* `Dose`
** Class encapsulating all the information about a given medication dosage e.g. dosage, dosage units, doses per day, etc.
* `Duration`
** Class encapsulating all the information about a given time period e.g. the start date, the end date, etc.
...
This command has the following intent: +
...
****
NOTE: If no/multiple patient(s) with that NRIC exist, then the `AddmedsCommand::execute` method will throw a `CommandException` and the usage case will end.
****
...
This updated `Person` object is used to update the existing `Person` object using the `Model::updatePerson` method. +
Finally, the `AddmedsCommand::execute` method returns a `CommandResult` with an appropriate message to the `LogicManager` instance, which returns the same `CommandResult` as the return value of the `LogicManager::execute` method. +
