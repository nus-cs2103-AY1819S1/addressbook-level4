...
In order to enable users to better manage their tasks in *Student Schedule Planner*, we added two more variations of
 ...
 supports viewing tasks from the current date till the closest Sunday.

==== Current Implementation

The `listday`/`listweek` mechanism utilise `DateSamePredicate`/`DateWeekSamePredicate` respectively.

When the `listday` command was executed, `DateSamePredicate` will use the current
system date as a 'filter' and
retrieve all tasks with deadline that match the system date.

Whereas for the `listweek` command, `DateWeekSamePredicate` will use the range of dates from current system date until
Sunday and retrieve all tasks with deadline that match the range of dates.

Given below is an example usage scenario and how the `listday`/`listweek` mechanism behaves at each step:

Step 1. The user entered the command `listweek`.

Step 2. `ListWeekCommand#appendDateList()` will generate the "list of days" in "ddmmyy" format.

Step 3. `model.updateFilteredTaskList()` will update the task list with `DateWeekSamePredicate`
as the parameter. `DateWeekSamePredicate` itself would take "list of days" in Step 2 as the paramter.
...
{to be added. Placeholder to avoid possible merge conflict}
