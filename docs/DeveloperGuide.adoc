...
It allows `.ics` files to be saved onto the computer. Calendars in Hallper are created as monthly calendars.
...
* `CalendarModel#createCalendar(Year year, Month month)` -- Initializes the calendar object in the `CalendarModel`.
...
These operations are exposed in the 'Model` interface as `Model#createCalendar(Year year, Month month)`, and in the `Storage` interface as `Storage#createCalendar(Calendar calendar, String calendarName)` respectively.
