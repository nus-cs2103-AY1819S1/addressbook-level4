...
range of index. The `DiscountItemCommand` object indirectly call `UniqueItemList#setItem(Item target, Item
editedItem)` to give the discount the `Item`. It will then save the current state for undo/redo.
...
_DiscountItem Activity diagram_
...
.. Incorrect commands to try: `edit-item 1 n/test p/string`, `edit-item n/@*@^&@* p/3`
...
.. Prerequisites: You must have at least 2 item in the menu.
.. Test case: `sort-menu name` +
   *Expected*: Sorts the menu by name. Sorting method shown in status message.

. Sorting the menu by price

.. Prerequisites: You must have at least 2 item with different prices in the menu.
.. Test case: `sort-menu price` +
   *Expected*: Sorts the menu by price. Sorting method shown in status message.

==== Giving a discount to an item

. Giving a discount to an item

.. Prerequisites: You must have at least 1 item in the menu.
.. Test case: `discount-item 1 dp/20` +
   *Expected*: Gives the item specified by the index a discount in the menu. Number of item given a discount shown in
    status message.
.. Incorrect commands to try: `discount-item -2 dp/20`, `discount-item string dp/20`

==== Adding recipe to an item
...
.. Prerequisites: You must have at least 1 item in the menu.
.. Test case: `add-required-ingredients 1 n/chicken nu/3` +
