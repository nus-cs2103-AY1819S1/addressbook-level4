...
When parsing the `Date` for `Reservation`, the `Date` class will check to see if the date entered is in the
`DD-MM-YYYY` format. If it is, then it will continue to parse the date normally using the parse method of `LocalDate`.
If not, it will use the `Natty` Library to parse the date value.

The `Date` class will also reject dates that have already passed the current date.
