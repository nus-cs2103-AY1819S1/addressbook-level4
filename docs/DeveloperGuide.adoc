...
3. In the `Storage`, the `LeaveApplication` will be copied and transformed to become an link:{repoURL}/blob/master/src/main/java/seedu/address/storage/XmlAdaptedLeaveApplication.java[`XmlAdaptedPerson`] object, which is then added into the link:{repoURL}/blob/master/src/main/java/seedu/address/storage/XmlAdaptedLeaveApplication.java[`XmlAdaptedPerson`] representing the person who applied for the leave, and finally saved into a file by the <<Design-Storage, Storage>> component.
