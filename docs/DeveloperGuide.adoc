...
The user can close the Schedular or use other functions of the Scheduler.

**Step 3**
At 6PM on that day, if the Scheduler is open, a pop-up alert will appear. The 'PopUpManager' pop out the corresponding `EventPopUpInfo`. Meanwhile, it will use the uid stored in `EventPopUpInfo` to find the `Event` object and set '4H: True'. +
The 8PM pop-up is still in the Pop-up queue.

==== Design Considerations
* **Use HashMap to store ReminderDurationList** (current choice)
** Pro: More difficult implementation
** Con: Faster searching when deleting
