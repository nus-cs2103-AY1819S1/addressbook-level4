...
This section documents how open/shutdown features are implemented and how they work.

==== Current Implementation

.Activity Diagram of `OpenCommandParser`
...
As seen in the diagram above, the `open` function is implemented with the help of `OpenCommand` and `UpdateRideDescriptor` class. When the
`ThaneParkParser` parses the user input and finds the open command word `open` it calls a new `OpenCommandParser` to
parse the arguments from the user input.
...
Finally, the `LogicManager` will execute the `open` command, and the `index` will be used to find the `ride` from the list.
