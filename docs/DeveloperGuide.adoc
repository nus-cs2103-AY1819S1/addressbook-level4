...
**Step 2** +
The user executes `GetGCEvents` command to download the Google Calendar Events to local database. +
...
The flowchar below shows the process of deciding whether to prompt a log in page or proceeds with local credential file.
...
**Step 5** + 
For each event extracted, a [proposed] eventConvertor utility method called `addGcEventToLocal` is called to convert the Google Extreacted Event to local Event.
