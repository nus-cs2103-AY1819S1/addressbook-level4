...
[NOTE]
There are only 2 valid contexts which a user can switch to with the command: 'volunteer' and 'event' with context ID of 'v' and 'e' respectively.

The `Switch` command updates the context found in `ModelManager` before raising the context change event to update the UI.

Given below is the sequence diagram to show how the `Switch` command works when switching from volunteer to event context.
...
** Pros: Can support even more context in the future due to the flexibility of a class.
** Cons: Tedious to do as a new class and relevant methods has to be implemented in model.
...
The manage command extends from the `Command` interface. It implements the following operation:

* `Command#execute()`: -- Executes the command and switches the current context to the 'manage' context.
