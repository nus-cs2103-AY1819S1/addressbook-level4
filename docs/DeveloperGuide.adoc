...
It is assumed that the user has already logged in to his/her account in Event Organiser.
...
Once again, note that we have assumed that the user has already logged in to his/her account in Event Organiser.
...
login/logout feature and to briefly explain the changes that were made on relevant
components of Event Organiser, namely, the EditUser Command, and the DeleteUser Command,
to ensure cohesiveness of the entire application.

.Sequence Diagram for the Logic component when the user issues an `login n/Alice pass/password` command.
...
[NOTE]
In this implementation, we define the uniqueness of a user by its tuple of NAME and PASSWORD. No two users can have the same NAME and PASSWORD pair. If two users have the same name, then they will have to use different passwords.

When the execute() method of the Login command is called,  the login command authenticates
the user using the autheneticateUser() method from the model component i.e. checks that the user that is trying to logging actually exists in Event Organiser.
A CommandException will be thrown in the following 2 scenarios:

* When the user does not exist in Event Organiser.
...
.Sequence Diagram for the Model component when the user issues an `login n/Alice pass/password` command.
...
.Sequence Diagram for the Model, EventsCenter, and UI component when the user issues an `login n/Alice pass/password` command.
...
For example, if the user, Alex Yeoh, successfully logs in to his account, he should see the welcome
message: "Welcome back Alex Yeoh!"

Second, upon successful login the colour of the name of the user will change from white to green.
The two images below attempts to illustrate the effects of a successful login on the UI. Again,
we will use the scenario that a user, Alex Yeoh, wants to login to his account:

Alex enters his name and password as required by the login command.

.UI before Alex Yeoh has successfully logs in.
...
The images below shows the Class Diagram for the Model component of the Login command.
Note that some minute details where ommited for the sake of simplicity.

image::LoginFeature_ClassDiagram_Model_1.png[width="800", margin-bottom="0"]

image::LoginFeature_ClassDiagram_Model_2.png[width="600", margin-top="0"]

image::LoginFeature_ClassDiagram_Model_3.png[width="800"]

.Class Diagram for the Model component of the Login Command
image::LoginFeature_ClassDiagram_Model_4.png[width="800"]


Again, note that some minute details for the Logic component where omitted for the sake of simplicity.
...
password. If two users have different names, they will be identified users. Even if two users have
the same name, they can simply choose different passwords for both users to use Event Organiser.
...
==== EditUser Command and DeleteUser Command
To ensure cohesiveness of the login/logout feature with the EditUser Command and DeleteUser
Command that were implemented in AddressBook-Level 4, appropriate changes was made to these
two commands.

First and foremost, we will need users to be logged in to edit or delete his/her user account.

Once the user issues an `deleteUser` command, it is automatically assumed that the user wants to
delete his/her own account. This ensures that only the user of the account is allowed to delete
his/her account.

Once the user issues an `editUser` command, it is also automatically assumed that the user wants to
delete his/her own account. This ensures that only the user of the account is allowed to edit
his/her account. However, due to the way users are uniquely identified in Event Organiser,
a user may find that they are unable to change their name or password. In these situations, the user
is encouraged to change their name or password to a different name or password respectively from
the new name or password that they have chosen before.
...
==== Current implementation
This section explains the implementation of the addition/deletion of friends feature associated with each user in EventOrganizer. The relevant commands which are callable by the user to be discussed are:
...
In Event Organiser, adding a user into the friend list is similar to following someone on Instagram. This is because having a person in your friend list will make it easier for you to be kept updated of his/her details, such as viewing his/her personal particulars or viewing the events that he/she had joined. This is because of the `ListFriendsCommand` which can display all the friends of the particular user and their details in the display panel as shown below.
...
The friend lists of each user have to be constantly updated when a user is deleted from the Event Organizer or his/her attributes are modified (such as change of name).
...
`Person` B's attributes gets modified (eg change name/phone):
...
=== Add a user
.. Test case: `addUser p\95642244 e\alex@exmple.com a\Blk 12 John Street, #10-11 pass\password`
      Expected: Add user unsuccessful.
.. Test case: `addUser n\Alex Yeoh e\alex@exmple.com a\Blk 12 John Street, #10-11 pass\password`
      Expected: Add user unsuccessful.
.. Test case: `addUser n\Alex Yeoh`
      Expected: Add user unsuccessful.
.. Test case: `addUser n\Alex Yeoh p\95642244 e\alex@exmple.com a\Blk 12 John Street, #10-11 pass\password`
   Expected: Add user successful.
