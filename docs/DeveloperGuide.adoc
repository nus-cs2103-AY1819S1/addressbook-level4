...
image::EmailActivityDiagram.png[width="1400"]
_Figure 4.2.1.1: Activity diagram of_ `ComposeEmailList`.
...
_Figure 4.2.1.2: Sequence diagram of_ `ComposeEmailList`.
...
*Step 1.* The user executes the `create_calendar` command by specifying the month and year.
...
*Step 2.* Once the calendar object is initialized in the `CalendarModel`, the `ModelManager` raises a `CalendarCreatedEvent`,
 to indicate that a calendar object has been initialized in the `CalendarModel`.

*Step 3.* The `CalendarCreatedEvent` goes to the `EventsCenter`, and is then handled by
...
image::CreateCalendarSeqDiagram.png[width="1500"]
_Figure 4.3.1.1: Sequence diagram for create_calendar command_
...
Both commands are facilitated by `ModelManager`, `CalendarModel` and `IcsCalendarStorage`. It implements the following operations:
...
*Step 1.* The user executes the `add_all_day_event` command by specifying the month, year, date and title.
...
*Step 3.* Once the calendar object is loaded in the `CalendarModel`, it then calls `Model#createAllDayEvent(Year year, Month month, int date, String title)`, which calls `CalendarModel#createAllDayEvent(Year year, Month month, int date, String title)`.
...
*Step 5.* The `AllDayEventAddedEvent` goes to the `EventsCenter`, and is then handled by
...
image::AddAllDayEventSeqDiagram.png[width="1500"]
_Figure 4.3.1.2: Sequence diagram for add_all_day_event command_
...
*Step 1.* The user executes the `add_event` command by specifying the month, year, starting date, starting time, ending date, ending time and title.
...
*Step 5.* The `CalendarEventAddedEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleCalendarEventAddedEvent(CalendarEventAddedEvent event)`, which then calls `IcsCalendarStorage#createCalendar(Calendar calendar, String calendarName)`.
This saves the updated calendar back to the local directory.

The following sequence diagram shows how the add_event operation works:

image::AddEventSeqDiagram.png[width="1500"]
_Figure 4.3.1.3: Sequence diagram for add_event command_


===== Delete_Event Command
This command is facilitated by `ModelManager`, `CalendarModel` and `IcsCalendarStorage`.
It implements the following operations:
...
*Step 1.* The user executes the `delete_event` command by specifying the month, year, starting date, ending date, and title.
...
*Step 5.* The `CalendarEventDeletedEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleCalendarEventDeletedEvent(CalendarEventDeletedEvent event)`, which then calls `IcsCalendarStorage#createCalendar(Calendar calendar, String calendarName)`.
This saves the updated calendar back to the local directory.

The following sequence diagram shows how the delete_event operation works:

image::DeleteEventSeqDiagram.png[width="1500"]
_Figure 4.3.1.4: Sequence diagram for delete_event command_
...
*Step 1.* The user executes the `view_calendar` command by specifying the month and year.
The `view_calendar` command then calls `Model#isExistingCalendar(Year year, Month month)`, to check whether the
calendar already exists inside Hallper. If it exists, the command does nothing and reflects to the user that the
calendar already exists. Else, the command calls `Model#loadCalendar(Year year, Month month)` which then raise a `LoadCalendarEvent`.

*Step 2.* The `LoadCalendarEvent` goes to the `EventsCenter`, and is then handled by `StorageManager#handleLoadCalendarEvent(LoadCalendarEvent event)`, which then calls `IcsCalendarStorage#loadCalendar(String calendarName)`.
This loads the calendar from the specified local directory.
...
image::ViewCalendarSeqDiagram.png[width="1500"]
_Figure 4.3.1.5: Sequence diagram for view_calendar command_

The following activity diagram summarizes what happens when a user executes a calendar command:

// tag::calendarDesign[]

image::CalendarCommandActivityDiagram.png[width="650"]
_Figure 4.3.1.6: Activity diagram for calendar commands_
...
each `Cca` contains the `Name` of the head and vice-head, `Budget` allocated, `Spent` and `Outstanding` and a set of
transaction `Entries`. Each `Entry` contains a `Date`, `Amount` and `Remarks`. The budget feature is facilitated by
...
the `Model` for the CCA and add the specified entry to the CCA. It then updates the `Cca` stored inside the `ccabook
.xml` file in the
local directory.
It implements the following commands:
...
*Step 6.* It then checks whether the CCA name is present in `BudgetWindow#fillInnerParts(CcaName ccaName)`. If CCA name
 ...
 `BudgetBrowserPanel()` will be called and a blank page will be showed.
...
_Figure 4.4.1.4: Activity diagram for budget command_
...
storing information.
** Cons: Parsing XML software is slow and cumbersome. Uses large amounts of memory due to the verbosity and incurs cost
 of parsing large XML files.
* **Alternative 2:** Save in `.json` format.
** Pros: Faster in parsing information as compared to `.xml`.
** Cons: JSON has no error handling. Therefore when the code fails to insert information, the code will not throw any
error.
...
_Figure 4.5.1.1: Sequence diagram of clear command on specified keywords._
...
_Figure 4.5.1.2: Activity diagram showing different path flows of clear command._
...
_Figure 4.6.1.1: Sequence diagram of erase command on specified keywords._
...
_Figure 4.6.1.2: Activity diagram showing different path flows of erase command._
...
_Figure 4.7.1.1: Sequence diagram of import command._
...
_Figure 4.7.1.2: Activity diagram showing different path flows of import command._
...
_Figure 4.8.1.1: Sequence diagram of export command._
...
_Figure 4.8.1.2: Activity diagram showing different path flows of export command._
...
image::Image Sequence Diagram.png[width="730"]
...
image::Image Activity Diagram.png[width="500"]
...
image::Search Sequence Diagram.png[width="600"]
...
image::Search Activity Diagram.png[width="500"]
...
_Table 5.4.1: List of site-wide attributes_
|===
...
[cols="1,2a,1", options="header"]
_Table 5.5.1: List of per-file attributes, excluding Asciidoctor's built-in attributes_
|===
|Attribute name |Description |Default value
...
2. Hallper requests for new details of the CCA (name of CCA, names of head and vice-head, budget, transaction entry
number, date, amount transaction remarks).
3. JCRC member enters the details.
...
=== Composing an email to all currently listed residents

. Composing an email while all persons are listed.

.. Prerequisites: List all persons using the `list` command. Multiple persons in the list.

.. Test case: `compose_email_list from/john@gmail.com subject/Example Subject content/Example Content` +
   Expected: Email displayed on the BrowserPanel with the emails of all listed contacts as recipients of the email.

.. Test case: `compose_email_list from/john@gmail.com subject/    content/Example Content` +
   Expected: Email not displayed on the BrowserPanel. Error details shown in the status message.

.. Test case: `compose_email_list from/john subject/Example Subject content/Example Content` +
   Expected: Email not displayed on the BrowserPanel. Error details shown in the status message.

. Composing an email while no persons are listed.

.. Prerequisites: Clear all persons by executing `clear all`. List should be empty.

.. Test case: `compose_email_list from/john@gmail.com subject/Example Subject content/Example Content` +
   Expected: Email not displayed on the BrowserPanel. Error details shown in the status message.

=== Composing an email to specified indexes

. Composing an email while all persons are listed.

.. Prerequisites: List all persons using the `list` command. Multiple persons in the list.

.. Test case: `compose_email_index from/john@gmail.com to/1 2 subject/Example Subject content/Example Content` +
   Expected: Email displayed on the BrowserPanel with the emails of the first and second contact as recipients of the email.

.. Test case: `compose_email_index from/john@gmail.com to/100 subject/Example Subject content/Example Content` +
   Expected: Email not displayed on the BrowserPanel. Error details shown in the status message.

.. Test case: `compose_email_index from/john@gmail.com to/0 subject/Example Subject content/Example Content` +
   Expected: Email not displayed on the BrowserPanel. Error details shown in the status message.
...
. Deleting an email that exists

.. Prerequisites: Email to delete exists in the directory in the computer.

.. Test case: `delete_email Valid Subject` +
   Expected: Email with subject 'Valid Subject' is deleted from the computer.

. Deleting an email that does not exist

.. Prerequisites: Email to delete does not exist in the directory in the computer.

.. Test case: `delete_email Invalid Subject` +
...
.. Test case: `list_emails` +
   Expected: Existing emails displayed on the BrowserPanel.

=== Viewing an email

. Viewing an email that exists

.. Prerequisites: Email to view exists in the directory in the computer.

.. Test case: `view_email Valid Subject` +
   Expected: Email displayed on the BrowserPanel.

. Viewing an email that does not exist

.. Prerequisites: Email to view does not exist in the directory in the computer.

.. Test case: `view_email Invalid Subject` +
   Expected: Email not displayed on the BrowserPanel. Error details shown in the status message.

=== Creating a monthly calendar
. Creating a new monthly calendar.

.. Test case: `create_calendar month/oct year/2018` +
   Expected: "Calendar created: OCT-2018.ics". The GUI remains at the same state as it was before the command execution.

.. Test case: `create_calendar month/october year/2018` +
...
. Creating an existing monthly calendar.
...
=== Viewing a monthly calendar
. Viewing an existing calendar.

.. Test case: `view_calendar month/oct year/2018` +
   Expected: The monthly calendar displays on the UI with the current week as its default time frame.

. Viewing a non-existing calendar.

.. Test case: `view_calendar month/sep year/1998` +
...
=== Adding an all day event into monthly calendar
* Adding a new all day event into the calendar.

.. Test case: `add_all_day_event month/oct year/2018 date/25 title/Example` +
   Expected: The updated calendar displays on the UI with the current week as its default time frame.

.. Test case: `add_all_day_event month/oct year/2018 date/-1 title/Example` +
   Expected: Error details shown in the status message. The GUI remains at the same state as it was before the command execution.
...
=== Adding an event into monthly calendar
* Adding a new event with a specified time frame into the calendar.

.. Test case: `add_event month/oct year/2018 sdate/1 shour/8 smin/0 edate/2 ehour/17 emin/30 title/Example` +
   Expected: The updated calendar displays on the UI with the current week as its default time frame.

.. Test case: `add_event month/oct year/2018 sdate/1 shour/24 smin/0 edate/2 ehour/17 emin/30 title/Example` +
   Expected: Error details shown in the status message. The GUI remains at the same state as it was before the command execution.

.. Test case: `add_event month/oct year/2018 sdate/1 shour/8 smin/0 edate/2 ehour/17 emin/60 title/Example` +
   Expected: Error details shown in the status message. The GUI remains at the same state as it was before the command execution.
...
=== Deleting an event from monthly calendar
* Deleting an existing event from the monthly calendar.

.. Test case: `delete_event month/oct year/2018 sdate/25 edate/25 title/example` +
...
=== Clearing persons
. Clearing persons while all persons are listed

.. Prerequitsites: List all persons using the `list` command. Multiple persons in the list.
.. Test case: `clear soccer` (where `soccer` exists in list) +
   Expected: Person(s) associated with `soccer` is/are cleared from the list.
.. Test case: `clear A123` (where `A123` exists in list) +
   Expected: Person(s) associated with `A123` is/are cleared from the list.
.. Test case: `clear soccer A123` (where `soccer` and/or `A123` exist in list) +
   Expected: Person(s) associated with `soccer` or `A123` is/are cleared from the list.
.. Test case: `clear invalidCCA` (where `invalidCCA` does not exist in list) +
   Expected: No person(s) are deleted. No persons cleared message displayed.

=== Erasing CCAs
. Erasing CCAs while all persons are listed

.. Prerequitsites: List all persons using the `list` command. Multiple persons in the list.
.. Test case: `erase basketball` (where `basketball` exists in list) +
   Expected: `basketball` is removed from associated person(s) in list.
.. Test case: `clear basketball soccer` (where `basketball` and `soccer` exist in list) +
...
=== Exporting data
. Exporting Hallper persons to the computer.

.. Prerequisites: `C://Users/Me/Documents` and `~/Desktop` are valid, accessible paths.
.. Test case: `export dst/C://Users/Me/Documents fn/export.xml` +
   Expected: Hallper data exported to `export.xml` in `C://Users/Me/Documents`.
.. Test case: `export dst/~/Desktop fn/export.xml` +
   Expected: Hallper data exported to `export.xml` in `~/Desktop`.
.. Test case: `export dst/~/Desktop fn/export.txt` +
   Expected: Data not exported. Invalid file type message displayed.

=== Importing data
. Importing data files from computer to update Hallper.

.. Prerequisites: Data files created in accessible path on computer with valid format and named `import.xml`.
   Refer to User Guide Section 4.1.9 for valid data formats. `C://Users/Me/Documents` and `~/Desktop` are valid,
   accessible paths.
.. Test case: `import dst/C://Users/Me/Documents/import.xml` +
   Expected: Data imported from `import.xml` in `C://Users/Me/Documents`.
.. Test case: `import f/~/Desktop/import.xml` +
   Expected: Data imported from `import.xml` in `~/Desktop`.
.. Test case: `import f/C://Users/Me/Documents/import.txt` +
   Expected: Data not imported. Invalid file type message displayed.

=== Searching persons

. Searching persons in Hallper.

.. Prerequisites: Multiple persons in Hallper.
.. Test case: `search soccer` (where a person has `soccer` tagged).
   Expected: Persons that are in `soccer` will be displayed in the Peron List Panel. Number of persons message displayed.
.. Test case: `search FoS` (where `FoS` is not tagged to any persons). +
   Expected: Person Panel List will be empty. 0 persons message displayed.

=== Uploading a profile picture

. Uploading of person profile picture from computer to Hallper.

.. Prerequisites: Image files created in accessible path on computer with valid format and named `image.jpg`. Refer
   to User Guide Section 4.1.2 for valid data formats. Hallper has at least 1 valid person.
.. Test case: `image r/A123 f/C://Users/Me/Documents/image.jpg` +
   Expected: Image file copied and pasted into Hallper `out/production/resources/profile_picture. The image is named
   after the person's room number.
.. Test case: `image r/A111 f/C://Users/Me/Documents/image.jpg` (where there is no resident tagged to `A111`). +
   Expected: Image not copied. No such person message displayed.
.. Test case: `image r/A123 f/NotAFilePath` +
   Expected: Image not copied. File path error message displayed.
.. Test case: `image r/A123 f/FileDontExist.jpg` +
   Expected: Image not copied. Invalid image error message displayed.

=== Creating CCA
. Adds a CCA with a given budget

.. Prerequisites: The name of the given CCA only contains alphabets. The budget given is numeric and the only symbol
can be used is dash.
.. Test case: `create n/Basketball budget/500` +
   Expected: New CCA added: Basketball Head: - Vice-Head: - Budget: 500
.. Test case: `create n/Basketball` +
   Expected: Invalid command format!
             create: Create a new CCA to the address book.
             Parameters: n/CCA NAME budget/BUDGET
             Example: create n/Basketball budget/500
.. Test case: `create` +
    Expected: Invalid command format!
              create: Create a new CCA to the address book.
              Parameters: n/CCA NAME budget/BUDGET
              Example: create n/Basketball budget/500
.. Test case: `create n/Basketball-M budget/500` +
    Expected: CCA names should only contain alphabet characters and spaces, and it should not be blank!
.. Test case: `create n/Basketball budget/$500`
    Expected: Budget should only contain numbers and it should not be blank.

=== Deleting CCA
. Deletes a CCA in Hallper.

.. Prerequisites: The specified CCA must exist in Hallper.
.. Test case: `delete_cca c/basketball` +
   Expected: Deleted CCA: Basketball Head: - Vice-Head: - Budget: 500
.. Test case: `delete_cca c/tennis` +
    Expected: The CCA does not exist.
.. Test case: `delete_cca` +
    Expected: Invalid command format!
              delete_cca: Deletes the CCA specified
              Parameters: c/CCA
              Example: delete_cca c/ basketball

=== Updating CCA
. Update the details of a CCA.

.. Prerequisites: The specified CCA must exist in Hallper. The fields provided must be in the correct format.
.. Test case: `update c/hockey h/Peter Park` +
   Expected: CCA updated: Hockey Head: Peter Park Vice-Head: Bernice Yu Budget: 500
.. Test case: `update c/tennis h/Peter Park` +
    Expected: The CCA does not exist.
.. Test case: `update c/hockey trans/1 amount/200` +
    Expected: CCA updated: Hockey Head: Peter Park Vice-Head: Bernice Yu Budget: 500

=== Adding a transaction entry
. Adds a transaction entry for a specified CCA in Hallper

.. Prerequisites: The specified CCA must exist in Hallper. The details of the transaction must all be provided and
are in the correct format.
.. Test case: `add_trans c/hockey date/20.12.2018 amount/-100 remarks/venue booking` +
   Expected: Transaction added: Hockey Head: Alex Yeoh Vice-Head: Bernice Yu Budget: 500
.. Test case: `add_trans c/tennis date/20.12.2018 amount/-100 remarks/venue booking` +
   Expected: The CCA does not exist. Please create the CCA before. adding its transaction
.. Test case: `add_trans c/tennis date/20.12.2018 amount/-100` +
    Expected: All the fields must be provided.
              add_trans: Add a new transaction to the specified Cca
              Parameters: c/CCA date/DD.MM.YYYY amount/AMOUNTremarks/REMARKS
              Example: add_trans c/basketball date/13.12.2018 amount/-100 remarks/Purchase of equipment

=== Deleting a transaction entry
. Deletes a transaction entry for a specified CCA in Hallper.

.. Prerequisites: The specified CCA must exist in Hallper. The transaction entry must exist for that CCA.
.. Test case: `delete_trans c/basketball trans/1` +
   Expected: Deleted Cca Transaction Entry:  Entry Number: 1 Date: 13.11.2013 Amount: -300 Remarks: Purchase of Equipment
.. Test case: `delete_cca c/basketball` +
    Expected: delete_trans: Deletes the specified transaction entry of the specified CCA
              Parameters: c/CCA INDEX (must be a positive integer)
              Example: delete_trans c/ basketball 2
.. Test case: `delete_trans c/tennis trans/1` +
    Expected: The CCA does not exist.

=== Showing CCAs' budget
. Displays the CCA information and its transaction history.

.. Prerequisites: The specified CCA must exist in Hallper.
.. Test case: `budget` +
   Expected: Display budget.
.. Test case: `budget c/basketball` +
   Expected: Display budget.
.. Test case: `budget c/tennis` +
    Expected: The CCA does not exist.
