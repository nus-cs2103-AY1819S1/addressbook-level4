...
This section will explain the main nature of design adopted by the system.
...
The __``Main`` component__ consists of only one class called link:{repoURL}/src/main/java/seedu/address/MainApp.java[`MainApp`]. It is responsible for,
...
The __``Commons`` component__ consists of classes used by multiple other components. Two of those classes play important roles at the architecture level.
...
The _User Interface_ (`UI`) component consists of a `MainWindow` that is made up of different parts, with the base class
link:{repoURL}/src/main/java/seedu/address/ui/Ui.java[`Ui.java`] using _JavaFx UI_ framework.
...
In general, this is the workflow of the _UI_ component:

. Execute user commands using the `Logic` component.
. Bind itself to some data in the `Model` so that the _UI_ components can auto-update when data in the `Model` change.
. Respond to events raised from various parts of the App and updates the _UI_ components accordingly.

All of these _UI_ parts, including the `MainWindow`, inherit from the abstract `UiPart` class. The layout for each
component is defined in matching `.fxml` file and can be found in the `src/main/resources/view` folder.

In short, the layout of the link:{repoURL}/src/main/java/seedu/address/ui/MainWindow.java[`MainWindow`]
is specified in link:{repoURL}/src/main/resources/view/MainWindow.fxml[`MainWindow.fxml`] etc.

[[Design-Logic]]
=== Logic component

The `Logic` component consist of a `LogicManager` that takes care of all the parse and exexcution of commands.
...
In general, this is the workflow of the _Logic_ component:
...
.  Return the command execution by encapsulating as a `CommandResult` object which is passed back to the `UI`.

Please refer to the Sequence Diagram below for an example of interactions within the `Logic` component
using the `execute("delete 1")` _API_ call.

.Interactions for execute operation
...
Each `Carpark` object has various attributes; `address`, `carparkNumber`, `coordinate`, `carparkType`, `freeParking`,
`lotsAvailable`, `nightParking`, `shortTerm`, `totalLots`, `typeOfParking`,
`tags`

* The `ModelManager` also manages a filtered list of `Person(?) Carpark` objects filtered from the `UniquePersonList`.
...
[NOTE]
As a more OOP model, we can store a `Tag` list in `Car Park Finder`, which `Carpark` can reference.
...
Please refer to the Sequence Diagram below for the `undo` operation.
...
.Summary for executing a new command
...
The **Activity Diagram** given above explains what happens when a user executes a `l`
command.
...
Please refer to the Sequence Diagram below for the `query` operation.
...
The following activity diagram summarizes the implementation of the
filter command.

.Activity Diagram for filter command
...
** Pros: Easy to implement, user friendly.
** Cons: Does not facilitate multiple layers of filtering.
