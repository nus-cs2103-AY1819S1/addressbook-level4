...
Step 2: The user executes `enqueuepatient 5` command to assign the 5th patient in the list into the queue. Now the queue has
1 patient.
...
Step 4: The user executes `enqueuepatient 2` to assign the 2dn patient in the displayed list into the queue. Now the queue has 2 patients.
Step 5: The user lists all patients in the queue by using `showqueue` command. All patients currently waiting in the queue are displayed.
