...
The `Document` abstract class represents all the documents that can be issued by the clinic. The `Receipt`,
 ...
 to henceforth as _document related commands_, all inherit from it. It can be thought of as a *formatting template*,
 specifying and organising the common information that all these documents must have. Shown below is a class diagram
 illustrating how `Document`, `Receipt`, `MedicalCertificate` and `ReferralLetter` relate to one another.

.Class diagram representing how the `Document` and _document related_ classes are implemented.
image::DocumentClassDiagram.png[width="650"]

As the template on which the documents are built from, the `Document` `abstract class` itself *cannot* be instantiated.
 The `Document` `abstract class` has the following key features:
...
image::DocumentActivityDiagram.png[width="650"]
...
Step 1. The resulting _document related object_ will call the `generateFile()` method, which will first make a unique `fileName` for the file to be created, identified by the type of `Document`, `ServedPatient's` `name` and `icNumber`.
``` Java
...
====
Step 2. The `DocumentTemplate.html` file is then converted into a `string`. It is a template with placeholder values prefixed by a `$` that will be replaced with the actual information pertaining to the `Served Patient's` consultation.
``` Java
...
```
Step 4. Now that the `HTML` template is a `string` with the placeholder values replaced by the corresponding information, a `BufferedOutputStream`
is used to write the `string` into the `HTML` file, concluding the generation of the `HTML` file.
