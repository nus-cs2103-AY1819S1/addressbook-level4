...
The `ModelManager` which implements the interface `Model`,
...
`AddCommand`m if they do not have a budget set, the application will prompt them to do so. After they do so,
...
==== Recurring budget
Every time a user log in and `Model#loadUserData()`, the application will check if the user has set a time where the
