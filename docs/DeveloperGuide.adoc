...
into the system. However, if there is duplicate, the system will thorw `DuplicateAssignmentException`.
...
1. When user executes `editassignment 1 -an ASSIGNMENTNAME` command, the command will be parsed. The given prefix and field indicate that the corresponding
field of `Assignment` should be edited. To do so, `EditAssignmentDescriptor` object will be produce. This object has the newly edited `Assignment` object.

2. After, the 'Permission' of the user will be check. If the user do not have the appropriate permission, error message will be displayed.

3. While if the user has the appropriate permission, the old `Assignment` object will be replaced with the new `Assignment` onject.
...
The view deleted employee archive feature is facilitated by `VersionedArchiveList` and `UniquePersonList.

Given below is an example usage scenario and how the `VersionedArchiveList` and `UniquePersonList` behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedArchiveList` will be initialized with the initial archive list state and an empty `UniquePersonList` is maintained.

Step 2. The user executes delete 3 command on the active list to delete the 3rd person in the active list. The Person object from the `UniquePersonList` in `VersionedAddressBook` will be transferred to the `UniquePersonList` in `VersionedArchiveList` and store the deleted Person's data.

Step 3. The user executes archive command to view the `UniquePersonList` in `VersionedArchiveList`.
