...
The `listappt` command searches for that land on the date that was entered by the user.
The filtered appointments are found in the found in `ModelManager`. The list instantiated by filtering the `UniqueAppointmentList`
...
** Pros: Allows more leeway for receptionist if a booking mistake was made rather than cancelling and adding
the mistake to the patient's appointment history.
