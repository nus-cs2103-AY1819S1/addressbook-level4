...
Step 4: The word of the day will be parsed by WordOfTheDayParser, calling `WordOfTheDayParser#parse()` The WordOfTheDayParser exists solely for the purpose of utilizing the existing functions made for parsing the online Dictionary and putting the word into `:Word` objects.
