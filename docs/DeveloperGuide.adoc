...
command is implemented such that it is able to create a `MedicalHistory` object for a specific `Patient` specified by
index.

Step 1. The user types in `add-medical-history` in the command box followed by the parameter `INDEX al/milk c/sub
healthy`

Step 2. Upon hitting enter, the `MedicalHistory` will be created and added to the storage of the system

==== Design Consideration
===== Aspect: Separate adding and editing
* Alternative 1 (Current choice): `AddMedicalHistoryCommand` is only used for initialization of medical history. The
editing is implemented as separated commands to facilitate easy modification of medical history.
