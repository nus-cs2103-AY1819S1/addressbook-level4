...
* *Skip to the previous track* : `prev`
* *Repeat track* : `repeat track`
* *Repeat playlist* : `repeat playlist`
* *Turn off the repeat mode* : `repeat off`
* *Shuffle the playlist* : `shuffle`
* *Turn off the shuffle mode* : `shuffle off`

=== Commands for managing playlists in library
* *List all playlists* : `playlist list`
* *Search for a playlist* : `playlist search q/QUERY` +
Eg. playlist search q/Fav
* *Create a playlist* : `playlist new p/PLAYLIST` +
Eg. playlist new p/Favourites
* *Delete a playlist* : `playlist del p/PLAYLIST` +
Eg. playlist del p/Favourites

=== Commands for managing tracks in playlist
* *Add a track into a playlist* : `track add p/PLAYLIST t/TRACK` +
Eg. track add p/Favourites n/Some Song.mp3
* *Delete a track from a playlist* : `track del p/PLAYLIST i/INDEX` +
