...
As delete is a critical command, elevation is required hence the password field. +
...
Change the current password of the app to `NEW_PASSWORD`. This ensures that critical commands such as `delete` and `reset` can be performed by authorized personnels only. +
