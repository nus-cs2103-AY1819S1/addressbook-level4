...
i.e. from Primary to Secondary. For such changes, see command: edit.
* For students advancing to Secondary 5, use command: edit.
...
Retrieve the total earnings from tuition fees within a range of date in the current year. +
...
* Students without tuition time slot(s) are ignored by the earnings function.
...
. Does not account for students whose time slots have been changed when calculating earnings from a period
before his/her time slot has been changed.
