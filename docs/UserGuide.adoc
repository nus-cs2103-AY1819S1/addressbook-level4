...
****
* Selects the task and loads the information the task at the specified `INDEX`.
...
Users automatically level up when on gaining enough xp to reach the next level. The maximum level is capped at lvl.5.
