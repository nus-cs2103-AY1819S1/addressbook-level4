...
A patients needs to see the doctor? Better get him into the queue using the
register command! +
Alias: `reg` +
...
A patient needs to be inserted into a certain position? Insert him into any position! +
Alias: `ins` +
...
Doctor is finally ready to the serve the next patient? Use the serve command!
Alias: `ser` +
Format: `serve`
...
An apple a day certainly is not enough! Time to get give the patient some medicine!  +
Alias: `dm` +
...
Clinic staff can use this command to generate a medical certificate for the Served Patient. +
Format: `mc INDEX`
...
Clinic staff can use this command to generate a receipt for the Served Patient. +
Alias: `rec` +
...
Clinic staff can use this command to generate a referral letter for the Served Patient. +
Alias: `ref` +
