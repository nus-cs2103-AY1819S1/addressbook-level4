...
Format: `edit IDENTIFIER [n/NAME] [p/phone] [e/EMAIL] [a/ADDRESS] [t/TAG]...`

****
* Edits the person with a name matching the IDENTIFIER. The identifier *​must uniquely identify a person.*
* If nobody matches the identifier, InsuRen will notify you and not carry out changes.
* If there are multiple contacts matching the identifier, InsuRen will notify you and not carry out changes.
* You can be more specific in the name identifier, but in the case that two people have exactly the same name, you should use the edit by index command.
****
