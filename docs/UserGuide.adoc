...
* `EDUCATIONAL_LEVEL` must be `Primary`, `Secondary` or `JC` followed by a number. +
e.g. `... el/Primary 3 ...` is valid; however, `... el/primary 3 ...`, `... el/University 3 ...` or other inputs like these are invalid
...
`add ... g/mid-test 100 [other grades]`
...
Returns a list of of students whose grades fall between the minimum and maximum grade(both inclusive). +
...
Format: `EditTime n/NAME ts/OLD_TIMESLOT ts/NEW_TIMESLOT`
