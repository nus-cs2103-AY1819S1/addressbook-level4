...
Adds a person to InsuRen. Tags cannot be `edit` or `delete` (case-insensitive). +
...
Edits an existing person in InsuRen by the displayed list's index or by an existing name. Tags cannot be `edit` or `delete` (case-insensitive). +
...
* If the day is invalid, an error will be thrown.
...
==== Edit a tag
Edit a tag, replacing all its occurrences with a new user-specified tag. Tags cannot be `edit` or `delete` (case-insensitive).
...
e.g. `delete 3` +
and `delete John Doe Kah Wai`
* *Edit* : `edit INDEX/EXISTING_NAME [n/NAME] [p/PHONE_NUMBER] [e/EMAIL] [a/ADDRESS] [t/TAG]...` +
e.g. `edit 2 n/James Lee e/jameslee@example.com` +
and `edit John Doe n/John Chan a/525 East 80th Street, New York`
