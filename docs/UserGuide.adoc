...
Adds a calendarEvent to the venue book +
...
Shows a list of all calendarEvents in the venue book. +
Format: `list`

=== Editing a calendarEvent : `edit`

Edits an existing calendarEvent in the venue book. +
...
Edits the description number and email venue of the 1st calendarEvent to be `91234567` and `johndoe@example.com` respectively.
* `edit 2 n/Betsy Crower t/` +
Edits the title of the 2nd calendarEvent to be `Betsy Crower` and clears all existing tags.

=== Locating calendarEvents by title: `find`
...
Deletes the specified calendarEvent from the venue book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd calendarEvent in the venue book.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd calendarEvent in the venue book.
* `find Betsy` +
`select 1` +
...
Restores the venue book to the state before the previous _undoable_ command was executed. +
Format: `undo`

[NOTE]
====
Undoable commands: those commands that modify the venue book's content (`add`, `delete`, `edit` and `clear`).
====
...
Clears all entries from the venue book. +
Format: `clear`
