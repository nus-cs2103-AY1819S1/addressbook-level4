...
Format: `generateTiming group/GROUP_NAME d/EVENT_DATE i/EVENT_INDEX`

Examples:

* `generateTiming group/CS2103Group d/2018-09-30 i/1`

[NOTE]
====
* This command only works with groups and will not work for an user's own personal events.
* If the group does not exist, an error message is displayed accordingly to indicate that this command cannot be parsed.
====
...
e.g. `showLocation 5`
* *Suggest Location* : `generateLocation d/EVENT_DATE i/EVENT_INDEX` +
