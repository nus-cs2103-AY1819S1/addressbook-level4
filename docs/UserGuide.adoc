...
****
* Selects the wish and loads the webpage at the `url` specific to that wish at the specified `INDEX`.
...
* If `AMOUNT` is negative, money will be removed from savings for that wish. +
* If `AMOUNT` brings the savings value to below 0, `AMOUNT` will not be accepted.
...
* If `AMOUNT` saved to `TO_WISH_INDEX` is greater than the amount needed to fulfil that wish, excess funds will be channelled to unusedFunds in WishBook.
