...
Format: +
`addperson n/NAME[NON-OPTIONAL] p/PHONE_NUMBER e/EMAIL_ADDRESS a/HOME_ADDRESS [t/TAG]...`

====
*Command Format*

* Persons with the same name and at least one same identity field (phone number or email) are considered to be the same person.
* Name must be specified to create a person contact.
* By default, Phone number, email address, home address an tag are empty.
====
...
Format: +
`addmodule mc/MODULE_CODE[NON_OPTIONAL] mt/MODULE_TITLE ay/ACADEMIC_YEAR sem/SEMESTER [t/TAG]...` +

====
*Command Format*

* Module with the same code, academic year and semester are considered to be the same module.
* ModuleCode must be specified to create a module.
* By default, Academic Year, ModuleTitle, Semester and Tag are empty strings.
* Valid semester number is 1-4. (3 and 4 refers to special term).
====
...
* Occasion with the same name and date are considered to be the same occasion.
* OccasionName must be specified to create an occasion. The length of Occasion Name should be restricted between 3 - 30.
* By default, Occasion date, Occasion location and Tag are empty.
====
...
* The xml file will be stored in [FILEPATH] if the export succeeds.
****
...
* `[FILEPATH]` should be a valid file path.
* `[FILEPATH]` should not contain any whitespace.
****
...
This command:
****
* Export data to txt file with predefined style and layout.
* The txt file will be stored in [FILEPATH] if the export succeeds.
****
