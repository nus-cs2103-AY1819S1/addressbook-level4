...
[NOTE]
The difference between `filter` and `find` is that `filter` is more for searching using numbers while
`find` uses strings of alphabetic characters.
...
* `filter w/ < 10 m/ >= 50` +
Filters attractions with waiting time less than 500 and maintenance of more than equals 50 days.
