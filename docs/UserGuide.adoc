...
=== Expense List / Statistics Panel (Red)
...
=== Notifications Panel (Green)

This is a feature which is work in progress.

=== Command Box and Console (Yellow)
...
The Console is where any success and error messages may appear which are relevant to the most recent command entered.
