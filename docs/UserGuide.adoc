...
[TIP]
This command cannot be called if no notes have been added to the patient. Use <<docAdd, `docAdd`>> command with prefix `n/` to add `NOTES`.
...
*A*: You are required to add notes to the Current Patient before using the `finish` command, use the `docAdd` command to add notes.
