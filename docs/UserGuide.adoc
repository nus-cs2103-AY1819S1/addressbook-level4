...
* The leave will be applied for all the dates specified under -da
...
e.g. sort -d
* *Select an employee* : `select INDEX` +
* *Search employees by name* : `find NAME` +
e.g. search Joshua
...
* *Change password* : `passwd`
* *View archived employees* : `archive`
* *Undo commands* : `undo`
* *Redo commands* : `redo`
* *Logout from the system* : `Logout`
* *Exit* : `exit`
...
* *Add employee* : `add -n NAME -e EMAIL -n NUMBER -s SALARY [-t PROJECT]` +
* *Delete employee*: `delete INDEX`
* *Admin permission*: `clear`
* *Editing an employee*: `edit INDEX [-n NAME] [-p PHONE] [-e EMAIL] [-a ADDRESS] [-s SALARY] [-t PROJECT]`
* *Assign permission*:
