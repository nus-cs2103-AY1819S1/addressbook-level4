...
|===
|Format| Abbreviation | Example(s)
|`undo`| `u`,`un`,`und` | `find hougang`,`list`,`undo` +
`fin hougang`,`fil n/`,`u`,`u`
|===
...
|===
|Format| Abbreviation | Example(s)
|`redo`| `r`,`re`,`red` | `find sengkang`, `undo`,`redo` +
`fin hougang`,`r`
|===
...
|===
|Format| Abbreviation | Example(s)
|`history`| `hi`,`his`,`hist`,`histo`,`histor` | `history` +
`hi`
|===
...
|===
|Format| Abbreviation | Example(s)
| `notify TIME_SECONDS` | `n`,`no`,`not`,`noti`, `notif` | `notify 60` +
`n 10`
|===
...
Display the full format of an executable command word. +
Format: enter `COMMAND_WORD`, followed by pressing kbd:[Tab] key. +
Function: press kbd:[Tab].key again to move to the next parameter.

Example:

key in `s` in command box and then press kbd:[Tab].
