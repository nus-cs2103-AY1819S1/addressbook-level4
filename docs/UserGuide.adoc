...
== User Interface
After logging in, the interface appear as such:
...
==== Statistics Panel mode
In Statistics Panel mode, the Left Panel contains an upper and lower sections, similar to what is shown below:
...
The lower section four of the category budgets that have been set using the `setCategoryBudget` command.
...
To toggle both, omit `n/NOTIFICATION_TYPE`.

There are two toggle options, `on` and `off`, which can be indication by 'on' or 'off' respectively in the `TOGGLE_OPTION` parameter. `TOGGLE_OPTION` is a required parameter.
