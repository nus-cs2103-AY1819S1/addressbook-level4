...
* Due Date: the date and time that a task is due
* Priority: the priority value that a task has
* Description: additional description regarding the task
* Labels/Tags: a list of labels that can be used to classify/identify the task

==== Derived fields
* Remaining time: the time remaining, from the current time, until the Due Date of the task.
* Status: the current status of the task, depending on wether it has been completed or not.
It is denoted by either "In Progress" or "Completed".
* Effective Due Date:
