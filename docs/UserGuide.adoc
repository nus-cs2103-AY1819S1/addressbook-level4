...
==== Search and display recipes: `search KEYWORD...`
...
==== Search recipes based on ingredients: `inventory`

By default, recipes are sorted with the ingredients in ingredient manger. That is, a recipe which uses most number of
 ingredients in ingredient manager is located at the top
User can add
additional
parameters to
widen search result.
Format: +
`recipe [NUMBER_OF_SERVINGS] must KEYWORD… optional KEYWORD…`

****
* "must" and "optional" keywords are optional. User can type both of them, one of them, or none of them(e.g. just key
 in
"inventory").
* Search result guarantees Keywords entered after “must”.
...
* Search recipes based on ingredients manager availability: `inventory [NUMBER_OF_SERVINGS] must KEYWORD… optional
KEYWORD… end`
