...
. Type `liststock` or `ls` into the command box as shown in <<Figure 10>>.

.Start up of CLInic
image::addmedicine-example-1.png[width="650"]

[start = 2]
. See <<Figure 11>> for the change of the patient panel to medicine panel.

.Result of entering `liststock` command
image::addmedicine-example-2.png[width="650"]

[start = 3]
. Enter the `am` command with the desired parameters as shown in <<Figure 12>>
