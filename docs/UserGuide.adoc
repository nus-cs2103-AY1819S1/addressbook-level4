...
For a dependency of a task A on task B, A is defined as the dependent task while task B is defined as the dependee task
Format:
...
[%hardbreaks]
Used with no arguments, `mode` displays the current game mode. If used with arguments, it sets the game mode and difficulty level.
...
Format: `mode [Optional: GAME_MODE] [Optional: GAME_DIFFICULTY]`, where:
	...
	Difficulty can only be specified if Mode has been specified.
