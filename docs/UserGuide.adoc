...
By default, the list has recipes that are stored in normal recipe list. User can filter, sort the recipe list with
based on ingredients. +
Format: +
`recipe NUMBER_OF_SERVINGS include [inventory] KEYWORD... prioritize [inventory] KEYWORD...` +
****
* "include" keyword filters the recipes based on following ingredients. +
* "prioritize" keyword sorts the recipes based on number of following ingredients it contains. Recipe containing the
most number of following ingredients would placed high in order. +
* "inventory" keyword either following "include" or "prioritize", provides all the ingreident in Ingredient
Manager
as parameter to keyword it follows. +
* "inventory" keyword is optional, but it should directly follow "include" keyword or "prioritize" keyword.
****

==== View needed amount of ingredients for a Recipe
For specific recipe, user can view how much ingredient she should prepare additionally, with select command following
 cross find command. It refers to Ingredient Manager to get the amount of ingredients the user currently have. +
Format: +
`select INDEX` +
****
* Amount of needed ingredients is calculated based on the information in Ingredient Manager. +
e.g. Number of Servings *
 Total Amount in Recipe - Amount that user already have(in Ingredient Manager)
****
...
****
* INDEX needs to be numeric more than 0 and be a valid index found on the list
****
