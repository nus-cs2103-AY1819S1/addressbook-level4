...
Creates a suggested location to meet up for an event (uniquely identified by date and event's index number).
...
* Event tags should only be alphanumeric (A-Z, a-z, 0-9) and hence should exclude any special characters or symbols such as '%', '*', '~'
====
...
e.g. `showLocation 5`
* *Suggest Location* : `generateLocation n/EVENT_NAME d/EVENT_DATE i/EVENT_INDEX` +
e.g. `generateLocation n/First Meeting d/2018-01-14 i/1`
