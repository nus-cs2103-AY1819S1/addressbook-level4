...
* **`add`**`n/John Doe p/98765432 e/johnd@example.com a/John street, block 123, #01-01` : adds a contact named `John Doe` to the ClinicIO Database.
...
* `list` +
`delete 2` +
Deletes the 2nd patient in the clinic database.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd patient in the clinic database.
* `find Betsy` +
`select 1` +
...
Restores the clinic database to the state before the previous _undoable_ command was executed. +
Format: `undo`

[NOTE]
====
Undoable commands: those commands that modify the clinic database's content (`add`, `delete`, `edit` and `clear`).
====
...
Clears all entries from the clinic database. +
Format: `clear`
...
e.g. `find James Jake`
* *List* : `list`
* *Display all queues: queues
e.g. `queues`
* *Display specific queue: queue QINDEX
e.g. `queue 2`
* *Enqueue a patient to a specific queue: enqueue INDEX QINDEX
e.g. `enqueue 3 to 1`
* *Dequeue a patient from a queue: dequeue QINDEX
e.g. `dequeue 4`
* *Help* : `help`
* *Select* : `select INDEX` +
e.g.`select 2`
