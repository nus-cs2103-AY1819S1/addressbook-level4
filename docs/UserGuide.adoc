...
* If you are not yet logged in, you can still view event details using `selectEvent`, and find events using `findEvent` and `findEventByTime`, and list events using `listEvent`. You can also use the `selectUser`, `findUser` and `listUser` commands.
...
* You can create a special TimePoll using `addTimePoll` after all participants have joined to decide on a suitable time.
...
* You can also search for other users using the `findUser` and `suggestFriendsByInterests` commands to see what events they have joined and join that event as well.
...
* You can also search for other users and add them as your friend using `addFriend`.
