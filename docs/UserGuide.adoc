...
//tag::sorting[]
[TIP]
All tasks will be *sorted chronologically by default*. Tasks that are due earlier will appear before tasks that are
due later. If *two tasks have the same deadline, they will be arranged according to priority*. 3 is the highest
priority, while 1 is the lowest. +
//end::sorting[]
...
====
[blue]`INDEX` refers to the index of the task in the displayed task list. Refer to the task according to its index
number.
The [blue]`INDEX` *must be a positive integer*(≥1)
...
[blue]`DDMMYY` refers to the date format of day, month and year. It must fulfil the following criteria:
...
====
[NOTE]
Category names may contain spaces. For example, [blue]`Steam`, [blue]`Steam2`,
