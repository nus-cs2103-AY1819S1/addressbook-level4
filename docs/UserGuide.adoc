...
Format: `generateLocation event/EVENT_NAME

Examples:

* `generateLocation event/First Meeting`
...
Format: `addEventTag [t/EVENT_TAG...]`
...
====
* Event dates should be in the format YYYY-MM-DD
* Event index is based on the displayed index of each event in the displayed list (see example below).
====

Example:

* `deleteEvent d/2018-10-14 i/1`

image::delete-event.PNG[Deleting an event]
...
* *Show Faculty Location* : `showLocation INDEX` +
e.g. `showLocation 5`
* *Suggest Location* : `generateLocation event/EVENT_NAME` +
e.g. `generateLocation event/First Meeting`
...
e.g. `deleteEvent d/2018-10-12 i/1`
