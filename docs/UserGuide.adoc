...
=== Assigning a patient into the queue : `enqueuepatient`

Assigns the patient into the queue based on the index number used in the displayed patient list.
...
* `list` +
`enqueuepatient 7` +
Selects the 7th patient in the displayed list of patients resulted from the `list` command.

* `find Logan` +
`enqueuepatient 1` +
Selects the 1st patient in the displayed list of patients whose names contain *Logan* (case insensitive) resulted from the `find Logan` command.

=== Showing all patients in the queue : `showqueue`
...
=== Removing a patient from the queue : `dequeue INDEX`

Removes a patient from the queue based on the index number used in the displayed patient list.
...
* `list` +
`dequeue 8` +
Removes the 8th patient in the list of displayed patients from the queue.

* `find Juliet` +
`dequeue 2` +
Removes the 2nd patient in the list of patients whose names contain *Juliet* (case insensitive) resulted from the `find Juliet` command.
