...
This guide provides an overview of ClinicIO and shows you how to install ClinicIO.
In addition, this guide provides an in-depth documentation for all the features.

This guide does not assume any prior technical knowledge. Check the <<FAQ>> section for frequent user issues.

== Introduction

ClinicIO is a desktop app to help you manage clinic with ease, efficient and simplicity.
You can use ClinicIO to manage patient record, manage the clinic queue, manage clinic's medical inventory, manage clinic appointment and provide statistics on ClinicIO. +
...
[NOTE]
You can move on to <<User Interface Walkthrough>> if you have install ClinicIO.
