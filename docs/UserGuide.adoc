...
There are two types of Find Commands: Global Find and Specific Find. They can be used in conjunction with one
another. +
...
NOTE: There is a 1 minute buffer time allocated for doctors after the end of their appointments. This means that
...
.Display available doctors at current datetime
...
.Display available doctors on 24th November
...
.Display available doctors base on the given date and time
