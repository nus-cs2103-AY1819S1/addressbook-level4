...
[start=13]
. Let's save our work before **DEEP FRY** the living daylights out of it! You can do that by typing `save arrangement.png` or if you're feeling adventurous just `save` will overwrite your original file irreversibly.
...
Format : `canvas auto-resize [ON|OFF]` -> Turns on/off the auto-resize for the canvas.
...
Format : `layer add [INDEX] (LAYER_NAME)` -> Selects the image at the provided index and adds it to the current canvas at the very top.
...
Format : `layer remove [INDEX]` -> Permanently removes a layer from canvas.
[WARNING]
====
...
Format : `layer select [INDEX]` -> Selects a layer to that all `convert` operations will work on.

==== Swaps the order of two layers: `layer swap [TO_INDEX] [FROM_INDEX]`
Format : `layer swap [TO_INDEX] [FROM_INDEX]` -> Changes the order of any two distinct layers.
...
Format : `layer position [POSITION]` -> Sets the x and y co-ordinates of the current layer, (0,0) is defined to be the top left of the screen.
