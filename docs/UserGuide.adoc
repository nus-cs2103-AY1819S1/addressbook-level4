...
Heard of the latest medicine that effectively cures ezcema?
...
[underline]#Add aspirin to CLInic:# +
...
[start = 2]
. See Figure 11 below for the change of the patient panel to medicine panel.

.Result of entering `liststock` command
image::addmedicine-example-2.png[width="650"]

[start = 3]
. Enter the `am` command with the desired parameters as shown in Figure 12 below.

.Entering the `am` command
...
See Figure 13 below for example.

.Successful addition of the medicine
