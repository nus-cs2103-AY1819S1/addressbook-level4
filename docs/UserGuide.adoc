...
* A `Note` usually consists of a `Message` and a `Prescription`. However, using this command does not allow you add any `Precription`, since it is not meant to be used during the doctor's consultation. Hence, using this command will just add a `Note` with the specified `Message`, but an empty `Prescription`.
****
