...
Shows a list of all calendarEvents in the address book. +
Format: `list`

=== Editing a calendarEvent : `edit`

Edits an existing calendarEvent in the address book. +
...
Edits the phone number and email address of the 1st calendarEvent to be `91234567` and `johndoe@example.com` respectively.
...
Deletes the specified calendarEvent from the address book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd calendarEvent in the address book.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd calendarEvent in the address book.
* `find Betsy` +
`select 1` +
