...
** *Justification*:
*** For convenience: Restaurants use a large variety of ingredients on a daily basis, making it difficult to know
immediately the availability of each ingredient. This feature reduces the need for restaurant managers to manually do
 stock taking of its restaurant's ingredients.
*** To prevent food wastage: Restaurants often stock up in fixed quantities of ingredients, regardless of the actual
availability in the pantry. During periods of poor business, there may be excessive ingredients in the
 pantry, leading to food wastage if these ingredients are perishable. This feature provides restaurant owners with more information about availability, so they only stock up what is necessary.
...
underlying architecture and the components `logic`, `storage` and `model`. To maintain good abstraction, it
 ...
 features eg menu and sales management. The `stockup-history` and
 `consumption-history` commands also required picking up knowledge about JavaFX and SceneBuilder to create the front-end user interfaces.

* *Minor enhancement*: Enhanced the existing tag feature by adding different colours for different tags
