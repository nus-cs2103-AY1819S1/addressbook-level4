...
The `Find` command helps users find a list of car parks within a specific
location. After which, users can use the `Filter` command
to narrow down the list of car parks to just those that fit their needs,
e.g. a user can use the `Filter` command to find a car park that
has both night parking and short-term parking. With our `Query` and `Notify`
feature, users can always received the most updated car park information!

Moreover, to decide on which car park to choose or whether to even drive,
users can use the `Calculate` command to calculate the cost of
parking at a particular car park during
a specified period of time.

The `Autocomplete` and `Abbreviation` features makes it even easier for
users to find a car park of their choice by making typing easier.
...
* *Minor enhancement*: Customised the list of command messages to different commands.
