...
Office Assistance Specialized Integration System (OASIS) is an all-in-one app for companies to perform employee management. Users who are fast typers will benefit due to it's access to all commands with a CLI . OASIS provides features to manage employee's leave applications, projects and user accounts for each employee.
...
** Highlights: I implemented a feature for `LogicManager` to have a List of interceptors, which are a `FunctionalInterface` that allows the processing of a Command. This allows the password changing command to work without adding excessive coupling between `Command` and `LogicManager`. Security for each user was essental to ensure that a user cannot read commands entered by another user. This was implemented by clearing user-specific data after logout.
