...
the information of the CCAs and their budget and finance. The tags used in the address book are used to indicate the
person's CCAs. The budget features also require in-depth understanding of the source code as it adds and delete CCAs
and transaction, which is similar to adding and deleting a person in an address book.
