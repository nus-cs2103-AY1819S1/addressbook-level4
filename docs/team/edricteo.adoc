...
This open-sourced project is in collaboration with other 4 other course mates from the module CS2103 (Introduction to Software Engineering).
...
It was developed for users who prefers to interact with a Command Line Interface (CLI) over a Graphical User Interface (GUI).
We have managed to contribute ~27,000 lines of code to a code base of 10,000 lines of code.
Currently, we are targeting National University of Singapore (NUS) undergraduates, and plan to extend to other local universities.
...
** Highlights: This enhancement puts confidentiality in high regard with multiple security concerns raised and discussed prior to implementation. The in-built security feature was also made robust to prevent frustration of the users.
