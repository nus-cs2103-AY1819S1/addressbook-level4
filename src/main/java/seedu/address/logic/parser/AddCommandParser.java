...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ...
        PriorityValue email = ParserUtil.parsePriorityValue(argMultimap.getValue(PREFIX_PRIORITY_VALUE).get());
        Description description = ParserUtil.parseDescription(argMultimap.getValue(PREFIX_DESCRIPTION).get());
        Set<Label> labelList = ParserUtil.parseLabels(argMultimap.getAllValues(PREFIX_LABEL));

        Task task = new Task(name, phone, email, description, labelList);
