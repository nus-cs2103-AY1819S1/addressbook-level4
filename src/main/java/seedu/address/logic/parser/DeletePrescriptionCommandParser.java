...
public class DeletePrescriptionCommandParser implements Parser<DeletePrescriptionCommand> {
    ...
    public DeletePrescriptionCommand parse(String args) throws ParseException {
        requireNonNull(args);
        ArgumentMultimap argMultiMap = ArgumentTokenizer.tokenize(args, PREFIX_INDEX, PREFIX_MEDICINE_NAME);

        if (!arePrefixesPresent(argMultiMap, PREFIX_INDEX, PREFIX_MEDICINE_NAME)
