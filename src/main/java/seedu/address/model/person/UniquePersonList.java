...
public class UniquePersonList implements Iterable<Person> {
     ...
     */
    public void setPerson(Person target1, Person editedPerson1, Person target2, Person editedPerson2) {
        requireAllNonNull(target1, editedPerson1, target2, editedPerson2);

        int index1 = internalList.indexOf(target1);
        if (index1 == -1) {
            throw new PersonNotFoundException();
        }

        if (!target1.isSamePerson(editedPerson1) && contains(editedPerson1)) {
            throw new DuplicatePersonException();
        }

        internalList.set(index1, editedPerson1);

        int index2 = internalList.indexOf(target2);
        if (index2 == -1) {
            throw new PersonNotFoundException();
        }

        if (!target2.isSamePerson(editedPerson2) && contains(editedPerson2)) {
            throw new DuplicatePersonException();
        }

        internalList.set(index2, editedPerson2);
    }
