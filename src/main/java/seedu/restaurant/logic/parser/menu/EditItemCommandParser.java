...
public class EditItemCommandParser implements Parser<EditItemCommand> {
    ...
    public EditItemCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_NAME).isPresent()) {
            editItemDescriptor.setName(ItemParserUtil.parseName(argMultimap.getValue(PREFIX_NAME).get()));
        }
        if (argMultimap.getValue(PREFIX_PRICE).isPresent()) {
            editItemDescriptor.setPrice(ItemParserUtil.parsePrice(argMultimap.getValue(PREFIX_PRICE).get()));
        }
