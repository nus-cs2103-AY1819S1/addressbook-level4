 ...
 * equality so as to ensure that the patient being added or updated is unique
 * in terms of identity in the UniquePatientList. However, the removal of a
 * patient uses Patient#equals(Object) so as to ensure that the patient with
...
public class UniquePatientList implements Iterable<Patient> {
    ...
    public void setPatient(Patient target, Patient editedPatient) {
        ...
        if (!target.isSamePatient(editedPatient) && contains(editedPatient)) {
            // boolean a = target.isSamePerson(editedPatient);
            // boolean b = contains(editedPatient);
            throw new DuplicatePatientException();
        }
