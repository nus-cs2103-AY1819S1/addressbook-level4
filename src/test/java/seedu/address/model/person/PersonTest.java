...
import static seedu.address.logic.commands.CommandTestUtil.VALID_PASSWORD_PASSWORD2;
...
public class PersonTest {
    ...
    public void isSamePerson() {
        ...
        // same name, same email, same password -> returns true
        editedAlice = new PersonBuilder(ALICE).withPhone(VALID_PHONE_BOB).withAddress(VALID_ADDRESS_BOB)
                .withTags(VALID_TAG_HUSBAND).build();
        assertTrue(ALICE.isSamePerson(editedAlice));

        // same name, same phone, same email, same password -> returns true
        ...
        editedAlice = new PersonBuilder(ALICE).withPassword(VALID_PASSWORD_PASSWORD2).build();
        assertFalse(ALICE.isSamePerson(editedAlice));
    }
