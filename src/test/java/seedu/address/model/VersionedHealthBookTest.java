...
public class VersionedHealthBookTest {

    private final ReadOnlyHealthBook addressBookWithAmy = new AddressBookBuilder().withPerson(AMY).build();
    private final ReadOnlyHealthBook addressBookWithBob = new AddressBookBuilder().withPerson(BOB).build();
    private final ReadOnlyHealthBook addressBookWithCarl = new AddressBookBuilder().withPerson(CARL).build();
    private final ReadOnlyHealthBook emptyAddressBook = new AddressBookBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedHealthBook versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedHealthBook versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedHealthBook.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedHealthBook.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedHealthBook versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedHealthBook copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedHealthBook differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedHealthBook differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedHealthBook versionedAddressBook,
                                             List<ReadOnlyHealthBook> expectedStatesBeforePointer,
                                             ReadOnlyHealthBook expectedCurrentState,
                                             List<ReadOnlyHealthBook> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new HealthBook(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyHealthBook expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new HealthBook(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlyHealthBook expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new HealthBook(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedHealthBook} with the {@code addressBookStates} added into it, and the
     * {@code VersionedHealthBook#currentStatePointer} at the end of list.
     */
    private VersionedHealthBook prepareAddressBookList(ReadOnlyHealthBook... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedHealthBook versionedAddressBook = new VersionedHealthBook(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedHealthBook versionedAddressBook, int count) {
