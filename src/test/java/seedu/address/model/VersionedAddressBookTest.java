...
public class VersionedAddressBookTest {
    ...
    private final ReadOnlySchedulePlanner addressBookWithAmy = new AddressBookBuilder().withPerson(AMY).build();
    private final ReadOnlySchedulePlanner addressBookWithBob = new AddressBookBuilder().withPerson(BOB).build();
    private final ReadOnlySchedulePlanner addressBookWithCarl = new AddressBookBuilder().withPerson(CARL).build();
    private final ReadOnlySchedulePlanner emptyAddressBook = new AddressBookBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedSchedulePlanner versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedSchedulePlanner versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedSchedulePlanner.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedSchedulePlanner.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedSchedulePlanner versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedSchedulePlanner copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedSchedulePlanner differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedSchedulePlanner differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedSchedulePlanner versionedAddressBook,
        ...
        assertEquals(new SchedulePlanner(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlySchedulePlanner expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new SchedulePlanner(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlySchedulePlanner expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new SchedulePlanner(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedSchedulePlanner} with the {@code addressBookStates} added into it, and the
     * {@code VersionedSchedulePlanner#currentStatePointer} at the end of list.
     */
    private VersionedSchedulePlanner prepareAddressBookList(ReadOnlySchedulePlanner... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedSchedulePlanner versionedAddressBook = new VersionedSchedulePlanner(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedSchedulePlanner versionedAddressBook, int count) {
