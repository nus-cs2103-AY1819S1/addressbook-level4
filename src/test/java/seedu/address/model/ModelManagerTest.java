...
public class ModelManagerTest {
    ...
    public void hasPerson_personNotInAddressBook_returnsFalse() {
        assertFalse(modelManager.hasCalendarEvent(ALICE));
    }

    @Test
    public void hasPerson_personInAddressBook_returnsTrue() {
        modelManager.addCalendarEvent(ALICE);
        assertTrue(modelManager.hasCalendarEvent(ALICE));
    }
    ...
    @Test
    public void equals() {
        Scheduler scheduler = new AddressBookBuilder().withPerson(ALICE).withPerson(BENSON).build();
        ...
        String[] keywords = ALICE.getName().fullTitle.split("\\s+");
        modelManager.updateFilteredCalendarEventList(new NameContainsKeywordsPredicate(Arrays.asList(keywords)));
