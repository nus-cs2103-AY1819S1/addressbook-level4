...
public class EditCommandParserTest {
    ...
    private static final String TAG_EMPTY = " " + PREFIX_DRUG_ALLERGY;
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_DRUG_ALLERGY_DESC, Tag.MESSAGE_TAG_CONSTRAINTS);
        ...
        // while parsing {@code PREFIX_DRUG_ALLERGY} alone will reset the drug allergies of the {@code Person}
        // being edited, parsing it together with a valid drug allergy results in error
        assertParseFailure(parser, "1" + DRUG_ALLERGY_DESC_ASPIRIN + DRUG_ALLERGY_DESC_PENICILLIN + TAG_EMPTY,
                Tag.MESSAGE_TAG_CONSTRAINTS);
        assertParseFailure(parser, "1" + DRUG_ALLERGY_DESC_ASPIRIN + TAG_EMPTY + DRUG_ALLERGY_DESC_PENICILLIN,
                Tag.MESSAGE_TAG_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_EMPTY + DRUG_ALLERGY_DESC_ASPIRIN + DRUG_ALLERGY_DESC_PENICILLIN,
                Tag.MESSAGE_TAG_CONSTRAINTS);
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + DRUG_ALLERGY_DESC_PENICILLIN
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + DRUG_ALLERGY_DESC_ASPIRIN;
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // drug allergies
        userInput = targetIndex.getOneBased() + DRUG_ALLERGY_DESC_ASPIRIN;
        descriptor = new EditPersonDescriptorBuilder().withTags(VALID_DRUG_ALLERGY_ASPIRIN).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_acceptsLast() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + DRUG_ALLERGY_DESC_ASPIRIN + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + DRUG_ALLERGY_DESC_ASPIRIN + PHONE_DESC_BOB + ADDRESS_DESC_BOB + EMAIL_DESC_BOB
                + DRUG_ALLERGY_DESC_PENICILLIN;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withAddress(VALID_ADDRESS_BOB)
                .withTags(VALID_DRUG_ALLERGY_ASPIRIN, VALID_DRUG_ALLERGY_PENICILLIN).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor);
