...
public class IndexTest {
    ...
    @Test
    public void equals() {
        final Index fifthEventIndex = Index.fromOneBased(5);

        // same values -> returns true
        assertTrue(fifthEventIndex.equals(Index.fromOneBased(5)));
        assertTrue(fifthEventIndex.equals(Index.fromZeroBased(4)));

        // same object -> returns true
        assertTrue(fifthEventIndex.equals(fifthEventIndex));

        // null -> returns false
        assertFalse(fifthEventIndex.equals(null));

        // different types -> returns false
        assertFalse(fifthEventIndex.equals(5.0f));

        // different index -> returns false
        assertFalse(fifthEventIndex.equals(Index.fromOneBased(1)));
    }
}
