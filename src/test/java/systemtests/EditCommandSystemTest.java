...
public class EditCommandSystemTest extends MeaningBookSystemTest {
    ...
    public void edit() {
        ...
        /* Case: edit a word with new values same as another word's values but with different phone and email
         * -> edited
         */
        index = INDEX_SECOND_PERSON;
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_BOB + PHONE_DESC_AMY + EMAIL_DESC_AMY
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        editedWord = new PersonBuilder(BOB).withPhone(VALID_PHONE_AMY).withEmail(VALID_EMAIL_AMY).build();
        assertCommandSuccess(command, index, editedWord);
        ...
        assertCommandFailure(EditCommand.COMMAND_WORD + " " + INDEX_FIRST_PERSON.getOneBased() + INVALID_ADDRESS_DESC,
                Meaning.MESSAGE_MEANING_CONSTRAINTS);
        ...
        /* Case: edit a word with new values same as another word's values but with different email -> rejected */
    ...
    private void assertCommandSuccess(String command, Index toEdit, Word editedWord,
        ...
        expectedModel.updateWord(expectedModel.getFilteredPersonList().get(toEdit.getZeroBased()), editedWord);
        expectedModel.updateFilteredWordList(PREDICATE_SHOW_ALL_PERSONS);
