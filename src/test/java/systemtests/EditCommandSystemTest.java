...
public class EditCommandSystemTest extends SchedulerSystemTest {
    ...
    public void edit() {
        ...
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_BOB + PHONE_DESC_AMY + EMAIL_DESC_AMY
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        editedCalendarEvent = new PersonBuilder(BOB).withPhone(VALID_PHONE_AMY).withEmail(VALID_EMAIL_AMY).build();
        assertCommandSuccess(command, index, editedCalendarEvent);
        ...
        editedCalendarEvent = new PersonBuilder(calendarEventToEdit).withTags().build();
        assertCommandSuccess(command, index, editedCalendarEvent);
    ...
    private void assertCommandSuccess(String command, Index toEdit, CalendarEvent editedCalendarEvent,
        ...
        expectedModel.updateCalendarEvent(expectedModel.getFilteredCalendarEventList().get(toEdit.getZeroBased()), editedCalendarEvent);
        expectedModel.updateFilteredCalendarEventList(PREDICATE_SHOW_ALL_CALENDAR_EVENTS);

        assertCommandSuccess(command, expectedModel,
                String.format(EditCommand.MESSAGE_EDIT_CALENDAR_EVENT_SUCCESS, editedCalendarEvent), expectedSelectedCardIndex);
    }
