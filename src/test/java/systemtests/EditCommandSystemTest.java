...
public class EditCommandSystemTest extends HealthBaseSystemTest {
    ...
    public void edit() {
        ...
        Person editedPerson = new PersonBuilder(BOB).withTags(VALID_DRUG_ALLERGY_PENICILLIN).build();
        ...
        assertTrue(getModel().getHealthBase().getPersonList().contains(BOB));
        index = INDEX_SECOND_PERSON;
        assertNotEquals(getModel().getFilteredPersonList().get(index.getZeroBased()), BOB);
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NRIC_DESC_CHARLIE + NAME_DESC_BOB
                + PHONE_DESC_BOB + EMAIL_DESC_BOB + ADDRESS_DESC_BOB + DRUG_ALLERGY_DESC_ASPIRIN
                + DRUG_ALLERGY_DESC_PENICILLIN;
        editedPerson = new PersonBuilder(BOB).withNric(VALID_NRIC_CHARLIE).build();
        assertCommandSuccess(command, index, editedPerson);

        /* Case: clear drug allergies -> cleared */
        index = INDEX_FIRST_PERSON;
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + " " + PREFIX_DRUG_ALLERGY.getPrefix();
        ...
        /* Case: filtered person list, edit index within bounds of HealthBase and person list -> edited */
        ...
        /* Case: filtered person list, edit index within bounds of HealthBase but out of bounds of person list
         * -> rejected
         */
        showPersonsWithName(KEYWORD_MATCHING_MEIER);
        ...
        index = INDEX_FIRST_PERSON;
        selectPerson(index);
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NRIC_DESC_AMY + NAME_DESC_AMY
                + PHONE_DESC_AMY + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + DRUG_ALLERGY_DESC_ASPIRIN;
        ...
        assertCommandFailure(EditCommand.COMMAND_WORD + " 0" + NRIC_DESC_BOB + NAME_DESC_BOB,
                String.format(Messages.MESSAGE_INVALID_COMMAND_FORMAT, EditCommand.MESSAGE_USAGE));

        /* Case: invalid index (-1) -> rejected */
        assertCommandFailure(EditCommand.COMMAND_WORD + " -1" + NRIC_DESC_BOB + NAME_DESC_BOB,
                String.format(Messages.MESSAGE_INVALID_COMMAND_FORMAT, EditCommand.MESSAGE_USAGE));

        /* Case: invalid index (size + 1) -> rejected */
        invalidIndex = getModel().getFilteredPersonList().size() + 1;
        assertCommandFailure(EditCommand.COMMAND_WORD + " " + invalidIndex + NRIC_DESC_BOB + NAME_DESC_BOB,
                Messages.MESSAGE_INVALID_PERSON_DISPLAYED_INDEX);

        /* Case: missing index -> rejected */
        assertCommandFailure(EditCommand.COMMAND_WORD + NRIC_DESC_BOB + NAME_DESC_BOB,
                String.format(Messages.MESSAGE_INVALID_COMMAND_FORMAT, EditCommand.MESSAGE_USAGE));
