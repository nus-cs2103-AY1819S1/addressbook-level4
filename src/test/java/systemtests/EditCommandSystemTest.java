...
public class EditCommandSystemTest extends AddressBookSystemTest {
    ...
    public void edit() {
        ...
        model.updatePerson(
                getModel().getFilteredPersonList().get(INDEX_FIRST_PERSON.getZeroBased()), editedRecipe);
        assertCommandSuccess(command, model, expectedResultMessage);
        ...
        Recipe recipeToEdit = getModel().getFilteredPersonList().get(index.getZeroBased());
        ...
        recipeToEdit = getModel().getFilteredPersonList().get(index.getZeroBased());
        ...
        /* Case: edit a recipe with new values same as another recipe's values but with different address -> rejected */
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB
                + ADDRESS_DESC_AMY + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        assertCommandFailure(command, EditCommand.MESSAGE_DUPLICATE_PERSON);

        /* Case: edit a recipe with new values same as another recipe's values but with different phone -> rejected */
        command = EditCommand.COMMAND_WORD + " " + index.getOneBased() + NAME_DESC_BOB + PHONE_DESC_AMY + EMAIL_DESC_BOB
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND + TAG_DESC_HUSBAND;
        assertCommandFailure(command, EditCommand.MESSAGE_DUPLICATE_PERSON);

        /* Case: edit a recipe with new values same as another recipe's values but with different email -> rejected */
    ...
    private void assertCommandSuccess(String command, Index toEdit, Recipe editedRecipe,
        ...
        expectedModel.updatePerson(expectedModel.getFilteredPersonList().get(toEdit.getZeroBased()), editedRecipe);
        expectedModel.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        assertCommandSuccess(command, expectedModel,
                String.format(EditCommand.MESSAGE_EDIT_PERSON_SUCCESS, editedRecipe), expectedSelectedCardIndex);
    }
